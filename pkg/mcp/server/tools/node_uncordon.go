...
// UncordonNode 将节点恢复为可调度状态（uncordon）
func UncordonNode() (mcp.Tool, func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error)) {
	tool := mcp.NewTool("node_uncordon",
		mcp.WithTitleAnnotation("恢复节点可调度"),
		mcp.WithDescription("将指定节点恢复为可调度状态（uncordon）"),
	...
	handler := func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if !node.Spec.Unschedulable {
			return &mcp.CallToolResult{
				Content: []mcp.Content{mcp.TextContent{Type: "text", Text: fmt.Sprintf("node '%s' is already uncordoned", nodeName)}},
			}, nil
		}
		...
		msg := fmt.Sprintf("node '%s' has been uncordoned successfully", nodeName)
		return &mcp.CallToolResult{
