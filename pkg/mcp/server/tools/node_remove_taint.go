...
func RemoveTaintFromNode() (mcp.Tool, func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error)) {
	tool := mcp.NewTool("node_remove_taint",
		mcp.WithTitleAnnotation("删除节点污点"),
		mcp.WithDescription("从指定节点移除一个污点"),
		mcp.WithString("nodeName", mcp.Required(), mcp.Description("节点名称")),
		mcp.WithString("key", mcp.Required(), mcp.Description("污点的 key")),
		mcp.WithString("value", mcp.Description("污点的 value（可选，若添加时未设则留空）")),
		mcp.WithString("effect", mcp.Required(), mcp.Description("污点效果")),
	...
	handler := func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if !found {
			return &mcp.CallToolResult{
				Content: []mcp.Content{mcp.TextContent{Type: "text", Text: fmt.Sprintf("taint '%s=%s:%s' not found on node '%s'", key, value, effect, nodeName)}},
			}, nil
		}
		...
		if err != nil {
			return mcp.NewToolResultError(fmt.Sprintf("failed to remove taint: %v", err)), nil
		}

		msg := fmt.Sprintf("taint '%s=%s:%s' removed from node '%s'", key, value, effect, nodeName)
		return &mcp.CallToolResult{
