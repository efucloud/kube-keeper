...
func setJSONCache(ctx context.Context, key string, value interface{}) {
	if config2.RedisClient == nil {
		return
	}
	...
	if err = config2.RedisClient.Set(ctx, key, data, config2.DefaultCacheTTL()).Err(); err != nil {
		config2.Logger.Warnf("redis set key %s failed: %v", key, err)
		return
	}
	config2.Logger.Debugf("cache set, key=%s, ttl=%s", key, config2.DefaultCacheTTL().String())
}
