...
type ClusterNamespaceAccountRoleResource struct {
	Svc services.ClusterNamespaceAccountRoleService
}

func (r ClusterNamespaceAccountRoleResource) AddWebService(ws *restful.WebService) {
	apiInfo := common.ApiInfo{}
	apiInfo.Tag = "cluster-namespace-account-role"
	apiInfo.Description = "集群用户命名空间角色"
	common.RegisterApiInfo(apiInfo)
	apiExtend := "/cluster-namespace-account-role"
	ws.Route(ws.POST(config2.ClusterNamespaceAPIPrefix+apiExtend).
		Doc("创建集群用户命名空间角色").
		Notes("创建集群用户命名空间角色").
		To(r.create).
		...
		Reads(dtos2.ClusterNamespaceAccountRoleCreate{}).
		Returns(http.StatusOK, "成功", dtos2.ClusterNamespaceAccountRoleDetail{}).
		...
		Metadata(config2.FrontApiTag, "createClusterNamespaceAccountRole"))
	ws.Route(ws.GET(config2.ClusterAPIPrefix+apiExtend).
		Doc("获取集群用户命名空间角色列表").
		Notes("获取集群用户命名空间角色信息").
		Param(ws.HeaderParameter(config2.AuthHeader, "请求Token")).
		Param(ws.PathParameter("cluster", "集群编码")).
		...
		Filter(filters2.ClientInfo).Filter(filters2.I18n).Filter(filters2.Log).Filter(filters2.Auth).Filter(filters2.Permission([]string{config2.SystemRoleAdmin})).
		Metadata(restfulspec.KeyOpenAPITags, apiInfo.Tags()).
		Metadata(config2.FrontApiTag, "listClusterNamespaceAccountRole"))
	ws.Route(ws.GET(config2.ClusterNamespaceAPIPrefix+apiExtend+"/{id}").
		Doc("获取集群用户命名空间角色详情").
		Notes("获取集群用户命名空间角色信息详情").
		Param(ws.HeaderParameter(config2.AuthHeader, "请求Token")).
		Param(ws.PathParameter("cluster", "集群编码")).
		Param(ws.PathParameter("namespace", "命名空间")).
		...
		Filter(filters2.ClientInfo).Filter(filters2.I18n).Filter(filters2.Log).Filter(filters2.Auth).Filter(filters2.Permission([]string{config2.SystemRoleAdmin})).
		Metadata(restfulspec.KeyOpenAPITags, apiInfo.Tags()).
		Metadata(config2.FrontApiTag, "getClusterNamespaceAccountRole"))
	ws.Route(ws.DELETE(config2.ClusterNamespaceAPIPrefix+apiExtend).
		Doc("删除集群用户命名空间角色").
		Notes("删除集群用户命名空间角色信息详情").
		Param(ws.HeaderParameter(config2.AuthHeader, "请求Token")).
		Param(ws.PathParameter("cluster", "集群编码")).
		Param(ws.PathParameter("namespace", "命名空间")).
		To(r.delete).
		...
		Filter(filters2.ClientInfo).Filter(filters2.I18n).Filter(filters2.Log).Filter(filters2.Auth).Filter(filters2.Permission([]string{config2.SystemRoleAdmin})).
		Metadata(restfulspec.KeyOpenAPITags, apiInfo.Tags()).
		Metadata(config2.FrontApiTag, "deleteClusterNamespaceAccountRole"))
}

func (r ClusterNamespaceAccountRoleResource) create(req *restful.Request, resp *restful.Response) {
	var (
		model       dtos2.ClusterNamespaceAccountRoleCreate
		errorData   common.ErrorData
		requestInfo structs.RequestInfo
		result      dtos2.ClusterNamespaceAccountRoleDetail
	)
	...
	result, errorData = r.Svc.AddClusterNamespaceAccountRole(ctx, requestInfo, model)
	if errorData.IsNotNil() {
...
func (r ClusterNamespaceAccountRoleResource) get(req *restful.Request, resp *restful.Response) {
	var (
		errorData   common.ErrorData
		result      dtos2.ClusterNamespaceAccountRoleDetail
		requestInfo structs.RequestInfo
	...
	result, errorData = r.Svc.GetClusterNamespaceAccountRoleById(ctx, requestInfo, id)
	if errorData.IsNotNil() {
...
func (r ClusterNamespaceAccountRoleResource) delete(req *restful.Request, resp *restful.Response) {
	var (
		errorData   common.ErrorData
	...
	svc.DeleteClusterNamespaceAccountRoleBinding(ctx, requestInfo, model.Ids)
	common.ResponseSuccess(resp, "删除成功")
}
func (r ClusterNamespaceAccountRoleResource) list(req *restful.Request, resp *restful.Response) {
	var (
		errorData common.ErrorData
		results   dtos2.ClusterNamespaceAccountRoleDetailList
	...
	results, errorData = r.Svc.ListClusterNamespaceAccountRole(ctx, requestInfo, page, size, order, queryParam.WhereQuery, queryParam.WhereArgs)
	if errorData.IsNotNil() {
