...
type ClusterNamespaceAccountRoleDetailList struct {
	//当前页数据
	Data []*ClusterNamespaceAccountRoleDetail `json:"data"`
	//数据库满足条件的数据总数
	Total int64 `json:"total"`
}

// ClusterNamespaceAccountRoleDetail 集群命名空间用户
type ClusterNamespaceAccountRoleDetail struct {
	//主键
...
type ClusterNamespaceAccountRoleCreate struct {
	//主键
	...
	AccountId string `gorm:"type:varchar(50);column:account_id;<-:create" json:"accountId" validate:"required" description:"所属用户ID"`
	//集群命名空间
	Namespace string `gorm:"type:varchar(255);column:namespace;<-:create" json:"namespace" validate:"required"  description:"集群命名空间"`
	//角色名称
	RoleName string `gorm:"type:varchar(50)" json:"roleName" validate:"required"  description:"角色名称"`
	//角色绑定名称
	BindingName string `gorm:"type:varchar(50)" json:"bindingName" validate:"required" description:"角色绑定名称"`
	//临时权限
	IsTemp bool `gorm:"column:is_temp" json:"isTemp" description:"临时权限"`
	//开始时间
	StartTime time.Time `gorm:"column:start_time" json:"startTime" validate:"required_if=IsTemp true" description:"开始时间"`
...
func (md *ClusterNamespaceAccountRoleCreate) Default(ctx context.Context) {
	...
	md.Hash = common.MD5V(fmt.Sprintf("%s-%s-%s-%s", md.ClusterId, md.AccountId, md.Namespace, md.RoleName))
}
func (md *ClusterNamespaceAccountRoleCreate) Validate(ctx context.Context) (err error) {
...
type ClusterNamespaceAccountRoleUpdate struct {
	//主键
...
func (md *ClusterNamespaceAccountRoleUpdate) Default(ctx context.Context) {
	md.UpdaterId = config.GetOperatorFromCtx(ctx)
	md.UpdatedAt = time.Now()
}
func (md *ClusterNamespaceAccountRoleUpdate) Validate(ctx context.Context) (err error) {
	validate := validator.New()
	lang := common.GetLangFromCtx(ctx, "")
	validate.RegisterTagNameFunc(common.TagNameI18N(lang))
	trans := common.LoadValidateTranslator(lang, validate)
