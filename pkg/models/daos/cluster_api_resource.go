...
type ClusterApiResource struct {
	...
	//内容
	Content datatypes.LongText `gorm:"column:content" json:"content" validate:"required" description:"ApiResource内容"`
	//内容hash
	Hash string `gorm:"type:varchar(255);column:hash" json:"-" description:"内容hash"`
}

func (md *ClusterApiResource) TableName() string {
	return models.ClusterApiResourceTableName
}
func (md *ClusterApiResource) Indexes() (results map[string][]string) {
	results = make(map[string][]string)
	return
}
func (md *ClusterApiResource) UniqueIndexes() (results map[string][]string) {
	results = make(map[string][]string)
