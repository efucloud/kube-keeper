...
const parseLegacyKubernetesView = (pathname: string) => {
  const namespaceMatch = pathname.match(
    /^\/kubernetes\/cluster\/([^/]+)\/namespace\/([^/]+)(\/.*)?$/
  );
  if (namespaceMatch) {
    return {
      cluster: namespaceMatch[1],
      namespace: namespaceMatch[2],
      pathname: `/kubernetes/namespace${namespaceMatch[3] || ""}`,
    };
  }

  const clusterMatch = pathname.match(/^\/kubernetes\/cluster\/([^/]+)(\/.*)?$/);
  if (clusterMatch) {
    return {
      cluster: clusterMatch[1],
      namespace: "",
      pathname: `/kubernetes/cluster${clusterMatch[2] || ""}`,
    };
  }
...
export const withKubernetesViewQuery = (
  pathname: string,
  cluster?: string,
  namespace?: string
): string => {
...
export const toKubernetesQueryRoute = (rawPath: string): string => {
  const [pathAndQuery, hash = ""] = rawPath.split("#");
  const [pathname, rawSearch = ""] = pathAndQuery.split("?");
  const searchParams = new URLSearchParams(rawSearch);
  const parsed = parseLegacyKubernetesView(pathname);
  ...
  if (parsed.cluster) {
    searchParams.set("cluster", parsed.cluster);
  }
  if (parsed.namespace) {
    searchParams.set("namespace", parsed.namespace);
  }

  return `${parsed.pathname}${buildQueryString(searchParams)}${
    hash ? `#${hash}` : ""
  }`;
};
...
export const getCurrentViewInfo = (): ViewInformation => {
  ...
  if (!cluster) {
    const legacyView = parseLegacyKubernetesView(window.location.pathname);
    if (legacyView) {
      cluster = legacyView.cluster;
      namespace = namespace || legacyView.namespace;
    }
  }

  info.isAdmin = !cluster;
  info.isCluster = !!cluster;
