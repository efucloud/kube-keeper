...
import type { IPersistentVolume, PersistentVolumeList } from 'kubernetes-models/v1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<IPersistentVolume[]>([]);
  const [dataSource, setDataSource] = useState<IPersistentVolume[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const formRef = useRef<ProFormInstance>(undefined);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchPersistentVolume, setPatchPersistentVolume] = useState<IPersistentVolume>();
  ...
  const listPersistentVolumes = async () => {
    setLoading(true);
    try {
      const params = { cluster, address: 'api/v1/persistentvolumes' } as Record<string, any>;
      ...
      const data = (await clusterGetProxy(params)) as PersistentVolumeList;
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'PersistentVolume';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: IPersistentVolume[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: IPersistentVolume) => {
      const params = {

        cluster,
        address: `api/v1/persistentvolumes/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: IPersistentVolume) => {
    const nodes = [
      {
        key: 'view-yaml',
        label: (
          <a
            onClick={() => {
              setPatchPersistentVolume(record);
  ...
  const columns: ProColumns<IPersistentVolume>[] = [
    {
      ...
      render: (dom, entity) => {
        return (
          <a
            onClick={() => {
              setPatchPersistentVolume(entity);
              setResourceDrawerVisible(true);
            }}
          >
    ...
    },
    {
      ...
      search: false,
      render: (dom, entity) => {
        return <>{entity?.spec?.capacity?.storage}</>;
      },
    },
    {
      ...
      key: 'status',
      search: false,
      render: (dom, entity: IPersistentVolume) => {
        // "Failed" | "Pending" | "Running" | "Succeeded" | "Unknown";
        const phase = entity?.status?.phase || 'Unknown';
        if (phase === 'Available') {
          return (
            <Tag style={{ border: 0 }} color="green">
              <FormattedMessage id="cluster.resource.persistentVolume.status.Available" />
            </Tag>
          );
        } else if (phase === 'Pending') {
          return (
            <Tag style={{ border: 0 }} color="orange">
              <FormattedMessage id="cluster.resource.persistentVolume.status.Pending" />
            </Tag>
          );
        } else if (phase === 'Bound') {
          return (
            <Tag style={{ border: 0 }} color="blue">
              <FormattedMessage id="cluster.resource.persistentVolume.status.Bound" />
            </Tag>
          );
        } else if (phase === 'Failed') {
          return (
            <Tag style={{ border: 0 }} color="red">
              <FormattedMessage id="cluster.resource.persistentVolume.status.Failed" />
            </Tag>
          );
        } else {
          return (
            <Tag style={{ border: 0 }}>
              <FormattedMessage id="cluster.resource.persistentVolume.status.Released" />
            </Tag>
          );
        }
      },
    },
    {
      ...
      search: false,
      render: (dom, entity) => {
        return <>{entity?.spec?.claimRef?.namespace}</>;
      },
    },
    {
      ...
      search: false,
      render: (dom, entity) => {
        return <>{entity?.spec?.claimRef?.name}</>;
      },
    },
    {
        ...
        const nodes = [
          <Popconfirm
            ...
            title={
              ...
              intl.formatMessage({ id: 'cluster.persistentVolume' }) + '【' + record.metadata?.name + '】'}
            onConfirm={() => {
          ...
          <Space separator={<Divider orientation="vertical" />}>
            ...
            <Access accessible={true} key={'create'}>
              <Button
                ...
                key="create"
                onClick={() => {
                  window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/storage/persistentvolume/create/text`);
                }}
              >
            ...
            onClick={() => {
              Modal.confirm({
                ...
                content: intl.formatMessage({
                  id: 'cluster.persistentVolume.delete.description',
                }),
                okText: intl.formatMessage({ id: 'pages.operation.confirm' }),
      ...
      <Drawer
        ...
        title={
          <>
            ...
            {patchPersistentVolume?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchPersistentVolume?.metadata?.resourceVersion || 'edit'}
          edit={false}
          address={`api/v1/persistentvolumes/${patchPersistentVolume?.metadata?.name}`}
          kind="PersistentVolume"
          name={patchPersistentVolume?.metadata?.name || ''}

          cluster={cluster}
          content={patchPersistentVolume}
        />
      </Drawer>
      <AICopilot
        view='list'
