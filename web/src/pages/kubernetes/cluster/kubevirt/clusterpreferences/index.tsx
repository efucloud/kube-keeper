...
const IndexPage: React.FC = () => {
  ...
  const baseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/kubevirt/clusterpreferences`);
  const [dataSource, setDataSource] = useState<IVirtualMachineClusterPreference[]>([]);
  const [loading, setLoading] = useState(false);

  const getStatusText = (resource?: IVirtualMachineClusterPreference) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IVirtualMachineClusterPreference[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'VirtualMachineClusterPreference',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IVirtualMachineClusterPreference>[] = [
    {
  ...
  return (
    <PageContainer header={{ breadcrumb: {} }} title={resourceLabel} subTitle={resourceLabel}>
      <ProTable<IVirtualMachineClusterPreference>
        actionRef={actionRef}
