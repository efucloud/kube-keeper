...
const IndexPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IVirtualMachineClusterInstancetype[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'VirtualMachineClusterInstancetype',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IVirtualMachineClusterInstancetype>[] = [
    {
  ...
  return (
    <PageContainer header={{ breadcrumb: {} }} title={resourceLabel} subTitle={resourceLabel}>
      <ProTable<IVirtualMachineClusterInstancetype>
        actionRef={actionRef}
