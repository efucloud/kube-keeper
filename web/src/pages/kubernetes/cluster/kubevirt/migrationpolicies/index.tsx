...
const IndexPage: React.FC = () => {
  ...
  const getStatusText = (resource?: IMigrationPolicy) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IMigrationPolicy[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'MigrationPolicy',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IMigrationPolicy>[] = [
    {
