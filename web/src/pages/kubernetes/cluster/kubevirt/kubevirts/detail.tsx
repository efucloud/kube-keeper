...
const DetailPage: React.FC = () => {
  ...
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'kubevirts', resolvedNamespace);
  ...
  useEffect(() => {
    ...
    load();
  }, [cluster, name, resourceGroup.groupVersion]);
    ...
    <PageContainer
      ...
      extra={[
        <Button
          key="yaml"
          icon={<EditOutlined />}
          onClick={() => (window.location.href = `${baseAddress}/${name}/update`)}
        >
    ...
    >
      <Card>
        <Descriptions column={2}>
          ...
          <Descriptions.Item label={intl.formatMessage({ id: 'cluster.resource.kind' })}>{formatResourceKind(intl, info?.kind || 'KubeVirt')}</Descriptions.Item>
          ...
          <Descriptions.Item label={intl.formatMessage({ id: 'cluster.resource.namespace' })}>{info?.metadata?.namespace || resolvedNamespace}</Descriptions.Item>
