...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<IValidatingWebhookConfiguration[]>([]);
  ...
  const [patchWebhookConfiguration, setPatchWebhookConfiguration] = useState<IValidatingWebhookConfiguration>();
  const [resourceDrawerVisible, setResourceDrawerVisible] = useState<boolean>(false);
  const [drawerSize, setDrawerSize] = useState<number>(800);

  const listValidatingWebhookConfigurations = async () => {
    setLoading(true);
    try {
      const params = { cluster, address: 'apis/admissionregistration.k8s.io/v1/validatingwebhookconfigurations' } as Record<string, any>;
      ...
      const data = (await clusterGetProxy(params)) as ValidatingWebhookConfigurationList;
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'ValidatingWebhookConfiguration';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: IValidatingWebhookConfiguration[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: IValidatingWebhookConfiguration) => {
      const params = { cluster, address: `apis/admissionregistration.k8s.io/v1/validatingwebhookconfigurations/${entity.metadata?.name}` };
  ...
  const columns: ProColumns<IValidatingWebhookConfiguration>[] = [
    ...
    },
    {
      ...
      render: (dom, entity: IValidatingWebhookConfiguration) => {
        ...
        const nodes = [
          ...
          <Popconfirm
            ...
            title={
              ...
              'IValidatingWebhookConfiguration' + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      ...
      title={intl.formatMessage({
        id: 'menu.config.validatingwebhookconfiguration',
      })}
      subTitle="ValidatingWebhookConfiguration"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<IValidatingWebhookConfiguration>
        key='validating-webhook-configuration'
        actionRef={actionRef}
        formRef={formRef}
        scroll={{ x: 'max-content' }}
        rowKey={(record: IValidatingWebhookConfiguration) =>
      ...
      >
        <ResourceEditor
          ...
          edit={false}
          address={`apis/admissionregistration.k8s.io/v1/validatingwebhookconfigurations/${patchWebhookConfiguration?.metadata?.name}`}
