...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<ClusterInterceptor[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchClusterInterceptor, setPatchClusterInterceptor] = useState<ClusterInterceptor>();
  const formRef = useRef<ProFormInstance>(undefined);
  const [dataSource, setDataSource] = useState<ClusterInterceptor[]>([]);
  ...
  const address = `apis/${resourceGroup?.groupVersion}/clusterinterceptors`;
  const intl = useIntl();
  ...
  const listClusterInterceptors = async () => {
    setLoading(true);
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'ClusterInterceptor';
      }
  ...
  useEffect(() => {
    listClusterInterceptors();

  }, []);
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: ClusterInterceptor[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: ClusterInterceptor) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/clusterinterceptors/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<ClusterInterceptor>[] = [
    {
      ...
      render: (dom, entity) => {
        return (
          <>
            <a
              onClick={() => {
                setPatchClusterInterceptor(entity);
...

    {
      ...
      render: (dom, entity: ClusterInterceptor) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.tekton.ClusterInterceptor.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('ClusterInterceptor') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
        ...
        if (!record?.metadata?.ownerReferences) {
          nodes.unshift(
            <a
              key="edit"
              onClick={() => {
                setPatchClusterInterceptor(record);
                setResourceDrawerVisible(true);
  ...
  return (
    <PageContainer
      ...
      title={intl.formatMessage({
        id: 'menu.DevOps.Tekton.clusterInterceptor',
      })}
      subTitle="ClusterInterceptor"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<ClusterInterceptor>
        key='cluster-interceptor'
        scroll={{ x: 'max-content' }}
        actionRef={actionRef}
        rowKey={(record: ClusterInterceptor) =>
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('ClusterInterceptor')}:&nbsp;&nbsp;
            {patchClusterInterceptor?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchClusterInterceptor?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/clusterinterceptors/${patchClusterInterceptor?.metadata?.name}`}
          kind="ClusterInterceptor"
          name={patchClusterInterceptor?.metadata?.name || ''}

          cluster={cluster}
          content={patchClusterInterceptor}
        />
      </Drawer>
      <AICopilot
        view='list'
