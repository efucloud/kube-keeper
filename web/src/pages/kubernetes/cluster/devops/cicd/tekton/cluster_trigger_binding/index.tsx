...
import { Button, Divider, Drawer, Modal, message, Popconfirm, Popover, Space, Tag, Typography } from 'antd';
import React, { useEffect, useRef, useState } from 'react';
const { Text } = Typography;
import dayjs from 'dayjs';
import type { IntlShape } from 'react-intl';
import type { ClusterTriggerBinding } from '@/k8s-models/tekton/triggers/triggers.tekton.dev/v1beta1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<ClusterTriggerBinding[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchClusterTriggerBinding, setPatchClusterTriggerBinding] = useState<ClusterTriggerBinding>();
  const formRef = useRef<ProFormInstance>(undefined);
  const [dataSource, setDataSource] = useState<ClusterTriggerBinding[]>([]);
  ...
  const listClusterTriggerBindings = async () => {
    setLoading(true);
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'ClusterTriggerBinding';
      }
  ...
  useEffect(() => {
    listClusterTriggerBindings();
  }, []);
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: ClusterTriggerBinding[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: ClusterTriggerBinding) => {
      const params = {
        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/clustertriggerbindings/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<ClusterTriggerBinding>[] = [
    {
      ...
      render: (dom, entity) => {
        return (
          <>
            <a
              onClick={() => {
                setPatchClusterTriggerBinding(entity);
...

    {
      ...
      render: (dom, entity: ClusterTriggerBinding) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.tekton.ClusterTriggerBinding.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('ClusterTriggerBinding') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
        ...
        if (!record?.metadata?.ownerReferences) {
          nodes.unshift(
            <a
              key="edit"
              onClick={() => {
                setPatchClusterTriggerBinding(record);
                setResourceDrawerVisible(true);
  ...
  return (
    <PageContainer
      ...
      title={intl.formatMessage({
        id: 'menu.DevOps.Tekton.clusterTriggerBinding',
      })}
      subTitle="ClusterTriggerBinding"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<ClusterTriggerBinding>
        key='cluster-trigger-binding'
        scroll={{ x: 'max-content' }}
        actionRef={actionRef}
        rowKey={(record: ClusterTriggerBinding) =>
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('ClusterTriggerBinding')}:&nbsp;&nbsp;
            {patchClusterTriggerBinding?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchClusterTriggerBinding?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/clustertriggerbindings/${patchClusterTriggerBinding?.metadata?.name}`}
          kind="ClusterTriggerBinding"
          name={patchClusterTriggerBinding?.metadata?.name || ''}

          cluster={cluster}
          content={patchClusterTriggerBinding}
        />
      </Drawer>
      <AICopilot
        view='list'
