...
import type { ClusterRole } from 'kubernetes-models/rbac.authorization.k8s.io/v1';
...
const [info, setInfo] = useState<ClusterRole>();
  ...
  const baseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/access/clusterroles`);
  const params = useParams();
  const mode = params.action === Update ? Update : Create;
  const name = mode === Create ? '' : params.name || '';

  const getInfo = async () => {
    const res = (await clusterGetProxy({ cluster, address: `${baseApi}/${name}` })) as ClusterRole;
    setInfo(res);
  };
