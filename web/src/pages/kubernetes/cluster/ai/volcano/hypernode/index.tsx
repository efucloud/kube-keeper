...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<HyperNode[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchqueue, setPatchqueue] = useState<HyperNode>();
  ...
  const [dataSource, setDataSource] = useState<HyperNode[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: HyperNode[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: HyperNode) => {
      const params = {
        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/hypernodes/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<HyperNode>[] = [
    {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.volcano.HyperNode.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('HyperNode') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchqueue?.metadata?.namespace}/hypernodes/${patchqueue?.metadata?.name}`}
          kind="HyperNode"
