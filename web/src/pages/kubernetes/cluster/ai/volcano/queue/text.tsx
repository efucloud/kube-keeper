...
const YamOrJsonForm: FC<Record<string, any>> = () => {
  ...
  const [info, setInfo] = useState<Queue>();
  ...
  const BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/ai/volcano/jobs`);
  ...
  const getInfo = async () => {
    ...
    const res = (await clusterGetProxy(params)) as Queue;
    setInfo(res);
  };
  useEffect(() => {
    if (name && name !== '' && name !== '-') {
