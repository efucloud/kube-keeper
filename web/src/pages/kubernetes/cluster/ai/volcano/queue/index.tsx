...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Queue[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchqueue, setPatchqueue] = useState<Queue>();
  ...
  const [dataSource, setDataSource] = useState<Queue[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Queue[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Queue) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/queues/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<Queue>[] = [
    ...
    {
      title: intl.formatMessage({ id: 'cluster.volcano.queue.status' }),
      dataIndex: 'status',
      search: false,
      render: (dom, entity) => {
    ...
    {
      title: intl.formatMessage({ id: 'cluster.volcano.queue.weight' }),
      dataIndex: 'status',
      search: false,
      render: (dom, entity) => {
        return <>{entity?.spec?.weight}</>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.volcano.queue.reclaimable' }),
      dataIndex: 'status',
      search: false,
      render: (dom, entity) => {
        if (entity?.spec?.reclaimable) {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.volcano.Queue.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('Queue') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchqueue?.metadata?.namespace}/queues/${patchqueue?.metadata?.name}`}
          kind="Queue"
