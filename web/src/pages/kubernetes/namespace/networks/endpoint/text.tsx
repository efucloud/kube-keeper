...
import type { IEndpoints } from 'kubernetes-models/v1';
...
const YamOrJsonForm: FC<Record<string, any>> = () => {
  ...
  const baseAddress = namespace ? toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/networks/endpoints`) : toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/networks/endpoints`);
  ...
  const getInfo = async () => {
    if (!namespace) return;
    const res = (await clusterGetProxy({ cluster, address: `${baseApi}/${name}` })) as IEndpoints;
    setInfo(res);
  };
