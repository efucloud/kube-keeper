...
import { IEndpoints } from 'kubernetes-models/v1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<IEndpoints[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const intl = useIntl();
  const address = namespace ? `api/v1/namespaces/${namespace}/endpoints` : `api/v1/endpoints`;
  const [selectedRow, setSelectedRow] = useState<IEndpoints>();
  ...
  const listEndpointss = async () => {
    ...
    try {
      ...
      const data = (await clusterGetProxy(params)) as EndpointsList;
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: IEndpoints[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: IEndpoints) => {
      const params = {

        cluster,
        address: `api/v1/namespaces/${entity.metadata?.namespace}/endpoints/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<IEndpoints>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/networks/endpoints`),
                );
              }}
            >
    ...
    >
      ...
      >
        <ResourceEditor
          ...
          edit={false}
          address={`api/v1/namespaces/${selectedRow?.metadata?.namespace}/endpoints/${selectedRow?.metadata?.name}`}
