...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Certificate[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchCertificate, setPatchCertificate] = useState<Certificate>();
  ...
  const [dataSource, setDataSource] = useState<Certificate[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Certificate[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Certificate) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/certificates/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<Certificate>[] = [
    ...
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificate.ready' }),
      search: false,
      render: (dom, entity: Certificate) => {
        const readyCondition = getConditionByType(entity, 'Ready');
        return <span>{formatConditionStatus(intl, readyCondition)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificate.readyDetail' }),
      search: false,
    ...
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificate.secretName' }),
      search: false,
      render: (dom, entity: Certificate) => {
        const certificate = entity as any;
        return <span>{formatDisplayValue(certificate?.spec?.secretName)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificate.dnsNames' }),
      search: false,
      render: (dom, entity: Certificate) => {
        const certificate = entity as any;
        return (
    ...
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificate.notAfter' }),
      search: false,
      render: (dom, entity: Certificate) => {
        const certificate = entity as any;
        return <span>{formatDisplayValue(certificate?.status?.notAfter)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificate.renewalTime' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: Certificate) => {
        const certificate = entity as any;
        return <span>{formatDisplayValue(certificate?.status?.renewalTime)}</span>;
      },
    },
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.certmanager.Certificate.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('Certificate') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    ...
    >
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('Certificate')}:&nbsp;&nbsp;
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchCertificate?.metadata?.namespace}/certificates/${patchCertificate?.metadata?.name}`}
          kind="Certificate"
