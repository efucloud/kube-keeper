...
import type { Issuer } from '@kubernetes-models/cert-manager/cert-manager.io/v1/Issuer';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Issuer[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchCertificate, setPatchCertificate] = useState<Issuer>();
  ...
  const resourceGroup = getClusterApiVersions(cluster, ['cert-manager.io/v1'], 'Issuer');
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/issuers` : `apis/${resourceGroup.groupVersion}/issuers`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/networks/certmanager/issuers`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/networks/certmanager/issuers`);
  }
  const intl = useIntl();
  const resourceLabel = formatResourceKind(intl, 'Issuer');
  ...
  const [dataSource, setDataSource] = useState<Issuer[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Issuer[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Issuer) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/issuers/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<Issuer>[] = [
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/networks/certmanager/issuers`),
                );
              }}
            >
    ...
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.issuer.ready' }),
      search: false,
      render: (dom, entity: Issuer) => {
        const readyCondition = getConditionByType(entity, 'Ready');
        return <span>{formatConditionStatus(intl, readyCondition)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.issuer.readyDetail' }),
      search: false,
    ...
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.issuer.type' }),
      search: false,
      render: (dom, entity: Issuer) => {
        return <span>{formatIssuerType(intl, getIssuerType(entity))}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.issuer.endpoint' }),
      search: false,
      render: (dom, entity: Issuer) => {
        return <span>{getIssuerEndpoint(entity)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.issuer.acmeEmail' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: Issuer) => {
        const issuer = entity as any;
        return <span>{formatDisplayValue(issuer?.spec?.acme?.email)}</span>;
      },
    },
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.certmanager.Issuer.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('Issuer') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchCertificate?.metadata?.namespace}/issuers/${patchCertificate?.metadata?.name}`}
          kind="Issuer"
