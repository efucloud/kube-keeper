...
import type { Challenge } from '@kubernetes-models/cert-manager/acme.cert-manager.io/v1/Challenge';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Challenge[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchCertificate, setPatchCertificate] = useState<Challenge>();
  ...
  const [dataSource, setDataSource] = useState<Challenge[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Challenge[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Challenge) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/challenges/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<Challenge>[] = [
    {
    ...
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.challenge.state' }),
      search: false,
      render: (dom, entity: Challenge) => {
        const challenge = entity as any;
        return <span>{formatCertManagerState(intl, challenge?.status?.state)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.challenge.statusDetail' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: Challenge) => {
        const challenge = entity as any;
        return <span>{formatDisplayValue(challenge?.status?.reason)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.challenge.type' }),
      search: false,
      render: (dom, entity: Challenge) => {
        const challenge = entity as any;
        return <span>{formatDisplayValue(challenge?.spec?.type)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.challenge.dnsName' }),
      search: false,
      render: (dom, entity: Challenge) => {
        const challenge = entity as any;
        return <span>{formatDisplayValue(challenge?.spec?.dnsName)}</span>;
      },
    },
    {
    ...
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.challenge.processing' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: Challenge) => {
        const challenge = entity as any;
        return <span>{formatBooleanValue(intl, challenge?.status?.processing)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.challenge.presented' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: Challenge) => {
        const challenge = entity as any;
        return <span>{formatBooleanValue(intl, challenge?.status?.presented)}</span>;
      },
    },
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.certmanager.Challengesacme.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('Challenge') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/acme.cert-manager.io/v1/namespaces/${patchCertificate?.metadata?.namespace}/challenges/${patchCertificate?.metadata?.name}`}
          kind="Challenge"
