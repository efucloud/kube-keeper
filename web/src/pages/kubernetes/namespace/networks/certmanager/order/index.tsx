...
import type { Order } from '@kubernetes-models/cert-manager/acme.cert-manager.io/v1/Order';
...
const IndexDashboard: React.FC = () => {
  ...
  const [patchCertificate, setPatchCertificate] = useState<Order>();
  ...
  const resourceGroup = getClusterApiVersions(cluster, ['acme.cert-manager.io/v1'], 'Order');
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/orders` : `apis/${resourceGroup.groupVersion}/orders`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/networks/certmanager/orders`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/networks/certmanager/orders`);
  }
  const intl = useIntl();
  const resourceLabel = formatResourceKind(intl, 'Order');
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Order[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Order) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/orders/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<Order>[] = [
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/networks/certmanager/orders`),
                );
              }}
            >
    ...
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.order.state' }),
      search: false,
      render: (dom, entity: Order) => {
        const order = entity as any;
        return <span>{formatCertManagerState(intl, order?.status?.state)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.order.statusDetail' }),
      search: false,
      ...
      render: (dom, entity: Order) => {
        ...
        return (
          <span>
            {formatDisplayValue(order?.status?.reason || order?.status?.failureTime)}
          </span>
        );
      },
    },
    ...
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.order.dnsNames' }),
      search: false,
      render: (dom, entity: Order) => {
        const order = entity as any;
        return <span>{formatCompactList(order?.spec?.dnsNames || [])}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.order.authorizations' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: Order) => {
        const order = entity as any;
        return <span>{formatDisplayValue(order?.status?.authorizations?.length ?? null)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.order.url' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: Order) => {
        const order = entity as any;
        return <span>{formatDisplayValue(order?.status?.url)}</span>;
      },
    },
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.certmanager.Order.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('Order') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/acme.cert-manager.io/v1/namespaces/${patchCertificate?.metadata?.namespace}/orders/${patchCertificate?.metadata?.name}`}
          kind="Order"
