...
import type { CertificateRequest } from '@kubernetes-models/cert-manager/cert-manager.io/v1/CertificateRequest';
...
import { CncfCertManager } from '@/utils/cncf';
import {
  formatBooleanValue,
  formatConditionReasonMessage,
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<CertificateRequest[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchCertificate, setPatchCertificate] = useState<CertificateRequest>();
  ...
  const resourceGroup = getClusterApiVersions(cluster, ['cert-manager.io/v1'], 'CertificateRequest');
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/certificaterequests` : `apis/${resourceGroup.groupVersion}/certificaterequests`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/networks/certmanager/certificaterequests`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/networks/certmanager/certificaterequests`);
  }
  const intl = useIntl();
  const resourceLabel = formatResourceKind(intl, 'CertificateRequest');
  ...
  const [dataSource, setDataSource] = useState<CertificateRequest[]>([]);
  ...
  const listServerDatas = async () => {
    ...
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'CertificateRequest';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: CertificateRequest[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: CertificateRequest) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/certificaterequests/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<CertificateRequest>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/networks/certmanager/certificaterequests`),
                );
              }}
            >
    ...
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificaterequest.ready' }),
      search: false,
      render: (dom, entity: CertificateRequest) => {
        const readyCondition = getConditionByType(entity, 'Ready');
        return <span>{formatConditionStatus(intl, readyCondition)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificaterequest.readyDetail' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: CertificateRequest) => {
        const readyCondition = getConditionByType(entity, 'Ready');
        return <span>{formatConditionReasonMessage(readyCondition)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificaterequest.approved' }),
      search: false,
      render: (dom, entity: CertificateRequest) => {
        const approvedCondition = getConditionByType(entity, 'Approved');
        return <span>{formatConditionStatus(intl, approvedCondition)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificaterequest.denied' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: CertificateRequest) => {
        const deniedCondition = getConditionByType(entity, 'Denied');
        return <span>{formatConditionStatus(intl, deniedCondition)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.issuerRef' }),
      search: false,
      render: (dom, entity: CertificateRequest) => {
        return <span>{formatIssuerRef(entity)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificaterequest.username' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: CertificateRequest) => {
        const certificateRequest = entity as any;
        return <span>{formatDisplayValue(certificateRequest?.spec?.username)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificaterequest.isCA' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: CertificateRequest) => {
        const certificateRequest = entity as any;
        return <span>{formatBooleanValue(intl, certificateRequest?.spec?.isCA)}</span>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.resource.certmanager.certificaterequest.failureTime' }),
      search: false,
      hideInTable: !expandInfo,
      render: (dom, entity: CertificateRequest) => {
        const certificateRequest = entity as any;
        return <span>{formatDisplayValue(certificateRequest?.status?.failureTime)}</span>;
      },
    },
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.certmanager.CertificateRequest.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('CertificateRequest') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    ...
    >
      <ProTable<CertificateRequest>
        key='certificate-request'
        actionRef={actionRef}
        formRef={formRef}
        scroll={{ x: 'max-content' }}
        rowKey={(record: CertificateRequest) =>
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('CertificateRequest')}:&nbsp;&nbsp;
            {patchCertificate?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchCertificate?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchCertificate?.metadata?.namespace}/certificaterequests/${patchCertificate?.metadata?.name}`}
          kind="CertificateRequest"
          name={patchCertificate?.metadata?.name || ''}

          cluster={cluster}
          content={patchCertificate}
        />
      </Drawer>
      <AICopilot
        view='list'
