...
const YamOrJsonForm: FC<Record<string, any>> = () => {
  ...
  return (<PageContainer
    ...
    subTitle={<>{getClusterResource('Pod')}<></>{info?.metadata?.name ? `:${info?.metadata?.name}` : ''}</>}
  >
    {mode === Update && info?.apiVersion &&
      <ResourceJsonOrYamlForm apiVersion='v1' key='pod' kind='Pod' scope='namespace' create={false} requestAPI={`${BaseApi}/${name}`} listPage={BaseAddress} detailPage={BaseAddress} namespace={namespace || ''} name={info?.metadata?.name || ''} cluster={cluster} content={info || {}} />
    }
    {mode === Create &&
      <ResourceJsonOrYamlForm apiVersion='v1' key='pod' kind='Pod' scope='namespace' create={true} requestAPI={BaseApi} listPage={BaseAddress} detailPage={BaseAddress} namespace={namespace || ''} name={''} cluster={cluster} content={{}} />
    }
  </PageContainer>);
}
