...
import type { IDeployment, DeploymentList } from 'kubernetes-models/apps/v1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<IDeployment[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchDeployment, setPatchDeployment] = useState<IDeployment>();
  ...
  const [dataSource, setDataSource] = useState<IDeployment[]>([]);
  ...
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/deployments` : `apis/${resourceGroup.groupVersion}/deployments`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/workload/deployments`);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: IDeployment[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: IDeployment) => {
      const params = { cluster, address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/deployments/${entity.metadata?.name}` };
  ...
  const moreItems = (record: IDeployment) => {
    const nodes = [
      {
        key: 'view-yaml',
        label: (
  ...
  const reDeploy = async (bodyData: IDeployment) => {
    await clusterDeleteProxy({ cluster, address: `apis/${resourceGroup.groupVersion}/namespaces/${bodyData.metadata?.namespace}/deployments/${bodyData.metadata?.name}` });
    ...
    await clusterPostProxy({ cluster, address: `apis/${resourceGroup.groupVersion}/namespaces/${requestData.metadata?.namespace}/deployments` }, requestData,);
    message.success(intl.formatMessage({ id: 'cluster.pages.operation.success' }));
    actionRef.current?.reload();
  };
  const columns: ProColumns<IDeployment>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/workload/deployments`),
                );
              }}
            >
            ...
            <>
              ...
              >
                <a
                  onClick={() => {
                    if (namespace) {
                      window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/workload/deployments/${entity?.metadata?.name}`);
                    } else {
                      window.open(
                        toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/workload/deployments/${entity?.metadata?.name}`),
                        '_blank',
                      );
                    }
                  }}
    ...
    },
    {
      ...
      search: false,
      render: (dom, entity: IDeployment) => {
        const available = entity?.status?.availableReplicas ?? 0;
        const desired = entity?.spec?.replicas ?? 0;
        return (
          <>
            <a style={{ color: available > 0 ? '#52c41a' : 'red' }}>
              {available}&nbsp;
            </a>
            {'/ ' + desired}
          </>
    ...
    },
    {
      ...
      render: (dom, entity: IDeployment) => {
        ...
        // 判断是否真的可用
        const totalReplicas = entity.spec?.replicas || 0;
        const readyReplicas = entity.status?.readyReplicas || 0;
        let statusColor: 'green' | 'orange' | 'red' = 'red';
        let statusTextId = 'cluster.resource.UnAvailable';

        if (readyReplicas === totalReplicas) {
          if (readyReplicas > 0) {
            statusColor = 'green';
            statusTextId = 'cluster.resource.Available';
          } else {
            statusColor = 'red';
            statusTextId = 'cluster.resource.UnAvailable';
          }
        } else if (readyReplicas > 0) {
          statusColor = 'orange';
          statusTextId = 'cluster.resource.SomeAvailable';
        } else {
          statusColor = 'red';
          statusTextId = 'cluster.resource.UnAvailable';
        }
        tags.push(
          <Tag style={{ border: 0 }} key="Status" color={statusColor}>
            <FormattedMessage id={statusTextId} />
          </Tag>,
        );
        // 如果 IDeployment 被暂停，额外标记
        if (entity.spec?.paused) {
          tags.push(
            <Tag style={{ border: 0 }} key="Paused" color="default">
              <FormattedMessage id="cluster.deployment.deploy.pause" />
            </Tag>,
          );
        }
        return (
            ...
            <div>
              {initContainersNumber > 0 && (
                <>
                  ...
                  <Space key={entity.metadata?.resourceVersion + 'init'} orientation='vertical' size='small'>
              ...
              {containersNumber > 0 && (
                <>
                  ...
                  <Space key={entity.metadata?.resourceVersion + 'biz'} orientation='vertical' size='small'>
    ...
    },
    {
              ...
              <Popover
                ...
                title={
                  <div>
                    ...
                    <Space key='search-popover' orientation='vertical' size='small'>
                      {keys?.map((key: string) => (
                        <>
      ...
      render: (dom, entity: IDeployment) => {
        ...
        if (expandInfo) {
          return (
            <Space key={entity.metadata?.resourceVersion} orientation='vertical' size='small'>
    ...
    },
    {
      ...
      render: (_, record) => {
        const nodes = [
          ...
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.deployment.delete.description',
            })}
            title={
        ...
        if (!record?.metadata?.ownerReferences) {
          nodes.unshift(
            <a
              key="edit"
              onClick={() => {
                window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${record?.metadata?.namespace}/workload/deployments/${record?.metadata?.name}/update`);
              }}
            >
        ...
        return <Space key='nodes'>{nodes}</Space>;
      },
    },
  ];
    ...
    >
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          key={patchModalKey}
          kind="Deployment"
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchDeployment?.metadata?.namespace}/deployments/${patchDeployment?.metadata?.name}`}
      ...
      {ownerVisible && <OwnerReferencesView
        ...
        ownerReferences={patchDeployment?.metadata?.ownerReferences || []}
      ...
      {imageVisible && <>
        <PatchImages
          ...
          key={patchDeployment?.metadata?.namespace + '-' + patchDeployment?.metadata?.name}
          ...
          containers={patchDeployment?.spec?.template?.spec?.containers || []}
          initContainers={patchDeployment?.spec?.template.spec?.initContainers || []}
        />
      </>}
      <AICopilot
        view='list'
