...
const YamOrJsonForm: FC<Record<string, any>> = () => {
  ...
  const BaseApi = `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/statefulsets`;
  const getInfo = async () => {
  ...
  return (<PageContainer
    ...
    subTitle={<>{getClusterResource('StatefulSet')}<></>{info?.metadata?.name ? `:${info?.metadata?.name}` : ''}</>}
  >
    <Card>
      {mode === Update && info?.apiVersion &&
        <ResourceJsonOrYamlForm apiVersion={resourceGroup.groupVersion} key='statefulset' kind='StatefulSet' scope='namespace' create={false} requestAPI={`${BaseApi}/${name}`} listPage={BaseAddress} detailPage={BaseAddress} namespace={namespace || ''} name={info?.metadata?.name || ''} cluster={cluster} content={info || {}} />
      }
      {mode === Create &&
        <ResourceJsonOrYamlForm apiVersion={resourceGroup.groupVersion} key='statefulset' kind='StatefulSet' scope='namespace' create={true} requestAPI={BaseApi} listPage={BaseAddress} detailPage={BaseAddress} namespace={namespace || ''} name={''} cluster={cluster} content={{}} />
      }
    </Card>
  </PageContainer>);
}
