import { PageContainer, ProForm, ProFormDigit, ProFormSelect, ProFormSwitch, ProFormText, ProFormTextArea } from '@ant-design/pro-components';
...
import { IStatefulSet } from 'kubernetes-models/apps/v1';
...
const AdvancedStepForm: FC<Record<string, any>> = () => {
  ...
  const [info, setInfo] = useState<IStatefulSet>({ apiVersion: resourceGroup.groupVersion, kind: 'StatefulSet', metadata: { namespace: namespace }, spec: { replicas: 1 } } as IStatefulSet);

  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${selectedNamespace}/workload/statefulsets`);
  ...
  const onFinish = async () => {
    await clusterPostProxy({ cluster, address: `apis/${resourceGroup.groupVersion}/namespaces/${info.metadata?.namespace}/statefulsets` }, info as KubernetesResource);
    setSuccess(true)
  }
          ...
          {success && <Result
            ...
            extra={
              <Fragment>
                <Button type="primary"
                  onClick={() => {
                    window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${info.metadata?.namespace}/workload/statefulsets/${info.metadata?.name}`)
                  }}
