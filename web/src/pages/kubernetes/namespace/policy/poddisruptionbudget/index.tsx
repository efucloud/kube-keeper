...
import { Button, Modal, message, Popconfirm, Popover, Select, Space, Tag, Typography, Empty, Divider, theme } from 'antd';
import React, { useEffect, useRef, useState } from 'react';
const { Text } = Typography;
import dayjs from 'dayjs';
import type { IPodDisruptionBudget, PodDisruptionBudgetList } from 'kubernetes-models/policy/v1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<IPodDisruptionBudget[]>([]);
  const [dataSource, setDataSource] = useState<IPodDisruptionBudget[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const formRef = useRef<ProFormInstance>(undefined);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const intl = useIntl();
  const resourceGroup = getClusterApiVersions(cluster, ['policy/v1'], 'PodDisruptionBudget');
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/poddisruptionbudgets` : `apis/${resourceGroup.groupVersion}/poddisruptionbudgets`;
  ...
  const listPodDisruptionBudgets = async () => {
    setLoading(true);
    try {
      ...
      const data = (await clusterGetProxy(params)) as PodDisruptionBudgetList;
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'PodDisruptionBudget';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: IPodDisruptionBudget[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: IPodDisruptionBudget) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/poddisruptionbudgets/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<IPodDisruptionBudget>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/policy/poddisruptionbudgets`),
                );
              }}
            >
    ...
    },
    {
      ...
      render: (dom, entity) => {
        return <a onClick={() => { window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/policy/poddisruptionbudgets/${entity?.metadata?.name}/update`); }}>{entity?.metadata?.name}</a>;
      },
    },
    {
      ...
      render: (dom, entity: IPodDisruptionBudget) => {
  ...
  return (
    <PageContainer
      ...
      subTitle={'PodDisruptionBudget'}
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<IPodDisruptionBudget>
        key='pdb'
        scroll={{ x: 'max-content' }}
        actionRef={actionRef}
        formRef={formRef}
        rowKey={(record: IPodDisruptionBudget) =>
          ...
          <Space separator={<Divider orientation="vertical" />}>
            ...
            <Access accessible={true} key={'create'}>
              <Button
                ...
                key="create"
                onClick={() => {
                  window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/policy/poddisruptionbudgets/create/text`);
                }}
              >
