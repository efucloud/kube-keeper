...
import { Button, Dropdown, Modal, message, Popconfirm, Popover, Select, Space, Tag, Typography, Empty, Divider, theme } from 'antd';
import React, { useEffect, useRef, useState } from 'react';
const { Text } = Typography;
import dayjs from 'dayjs';
import type { HorizontalPodAutoscalerList, IHorizontalPodAutoscaler } from 'kubernetes-models/autoscaling/v1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<IHorizontalPodAutoscaler[]>([]);
  ...
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/horizontalpodautoscalers` : `apis/${resourceGroup.groupVersion}/horizontalpodautoscalers`;
  ...
  const listHorizontalPodAutoscalers = async () => {
    setLoading(true);
    try {
      ...
      const data = (await clusterGetProxy(params)) as HorizontalPodAutoscalerList;
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'HorizontalPodAutoscaler';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: IHorizontalPodAutoscaler[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: IHorizontalPodAutoscaler) => {
      const params = { cluster, address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/horizontalpodautoscalers/${entity.metadata?.name}` };
  ...
  const moreItems = (record: IHorizontalPodAutoscaler) => {
    const nodes = [];
    nodes.push({
      key: 'annotation',
      label: (
        <a onClick={() => { }} style={{ color: colorPrimary }}>
  ...
  const columns: ProColumns<IHorizontalPodAutoscaler>[] = [
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/policy/hpa`),
                );
              }}
            >
    ...
    },
    {
      ...
      render: (dom, entity) => {
        return <a onClick={() => { window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/policy/hpa/${entity?.metadata?.name}/update`); }}>{entity?.metadata?.name}</a>;
      },
    },
    {
      ...
      render: (dom, entity: IHorizontalPodAutoscaler) => {
  ...
  return (
    <PageContainer
      ...
      subTitle={'HorizontalPodAutoscaler'}
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<IHorizontalPodAutoscaler>
        key='hpa'
        scroll={{ x: 'max-content' }}
        actionRef={actionRef}
        rowKey={(record: IHorizontalPodAutoscaler) =>
          ...
          <Space separator={<Divider orientation="vertical" />}>
            ...
            <Access accessible={true} key={'create'}>
              <Button
                ...
                key="create"
                onClick={() => {
                  window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/policy/hpa/create/text`);
                }}
              >
