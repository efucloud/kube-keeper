...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<JobFlow[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchJob, setPatchJob] = useState<JobFlow>();
  ...
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/jobflows` : `apis/${resourceGroup.groupVersion}/jobflows`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/ai/volcano/jobflows`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/ai/volcano/jobflows`);
  }
  const intl = useIntl();
  ...
  const [dataSource, setDataSource] = useState<JobFlow[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: JobFlow[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: JobFlow) => {
      const params = { cluster, address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/jobflows/${entity.metadata?.name}` };
  ...
  const columns: ProColumns<JobFlow>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/ai/volcano/jobflows`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.volcano.JobFlow.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('JobFlow') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchJob?.metadata?.namespace}/jobflows/${patchJob?.metadata?.name}`}
          kind="JobFlow"
