...
import { Button, Divider, Drawer, Modal, message, Popconfirm, Popover, Select, Space, Tag, Typography, Empty, theme } from 'antd';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<PodGroup[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchJob, setPatchJob] = useState<PodGroup>();
  ...
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/podgroups` : `apis/${resourceGroup.groupVersion}/podgroups`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/ai/volcano/podgroups`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/ai/volcano/podgroups`);
  }
  const intl = useIntl();
  ...
  const [dataSource, setDataSource] = useState<PodGroup[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: PodGroup[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: PodGroup) => {
      const params = { cluster, address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/podgroups/${entity.metadata?.name}` };
  ...
  const columns: ProColumns<PodGroup>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/ai/volcano/podgroups`),
                );
              }}
            >
    ...
    {
      title: intl.formatMessage({ id: 'cluster.volcano.podgroup.queue' }),
      dataIndex: 'queue',
      search: false,
      render: (dom, entity) => {
        return <>{entity?.spec?.queue}</>;
      },
    },
    {
      title: intl.formatMessage({ id: 'cluster.volcano.podgroup.status' }),
      search: false,
      render: (dom, entity: PodGroup) => {
        // "Failed" | "Pending" | "Running" | "Succeeded" | "Unknown";
        const phase = entity?.status?.phase || 'Unknown';
        if (phase === 'Running') {
          return (
            <Tag style={{ border: 0 }} color="green">
              <FormattedMessage id="cluster.volcano.podgroup.status.Running" />
            </Tag>
          );
        } else if (phase === 'Inqueue') {
          return (
            <Tag style={{ border: 0 }} color="orange">
              <FormattedMessage id="cluster.volcano.podgroup.status.Inqueue" />
            </Tag>
          );
        } else if (phase === 'Pending') {
          return (
            <Tag style={{ border: 0 }} color="orange">
              <FormattedMessage id="cluster.volcano.podgroup.status.Pending" />
            </Tag>
          );
        } else {
          return (
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.volcano.PodGroup.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('PodGroup') + '【' + record.metadata?.name + '】'}
            onConfirm={() => {
  ...
  return (
    ...
    >
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('PodGroup')}:&nbsp;&nbsp;
            {patchJob?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchJob?.metadata?.namespace}/podgroups/${patchJob?.metadata?.name}`}
          kind="PodGroup"
