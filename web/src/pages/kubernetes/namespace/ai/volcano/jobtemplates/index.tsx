...
import type { JobTemplate } from '@/k8s-models/volcano/flow.volcano.sh/v1alpha1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<JobTemplate[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchJob, setPatchJob] = useState<JobTemplate>();
  ...
  const resourceGroup = getClusterApiVersions(cluster, ['flow.volcano.sh/v1alpha1'], 'JobTemplate');
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/jobtemplates` : `apis/${resourceGroup.groupVersion}/jobtemplates`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/ai/volcano/jobtemplates`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/ai/volcano/jobtemplates`);
  }
  const intl = useIntl();
  ...
  const [dataSource, setDataSource] = useState<JobTemplate[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: JobTemplate[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: JobTemplate) => {
      const params = { cluster, address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/jobtemplates/${entity.metadata?.name}` };
  ...
  const columns: ProColumns<JobTemplate>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/ai/volcano/jobtemplates`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.volcano.JobTemplate.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('JobTemplate') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    ...
    >
      <ProTable<JobTemplate>
        key='job-template'
        actionRef={actionRef}
        formRef={formRef}
        scroll={{ x: 'max-content' }}
        rowKey={(record: JobTemplate) =>
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('JobTemplate')}:&nbsp;&nbsp;
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchJob?.metadata?.namespace}/jobtemplates/${patchJob?.metadata?.name}`}
          kind="JobTemplate"
