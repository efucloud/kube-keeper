...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Command[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchJob, setPatchJob] = useState<Command>();
  ...
  const [dataSource, setDataSource] = useState<Command[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Command[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Command) => {
      const params = { cluster, address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/commands/${entity.metadata?.name}` };
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.volcano.Command.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('Command') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchJob?.metadata?.namespace}/commands/${patchJob?.metadata?.name}`}
          kind="Command"
