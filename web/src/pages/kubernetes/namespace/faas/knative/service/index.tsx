...
import { CncfFission, CncfKnative } from '@/utils/cncf';
...
const ServiceIndex: React.FC = () => {
  ...
  const [patchService, setPatchService] = useState<Service>();
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Service[]) => {
    ...
    selectedRows.forEach(async (entity: Service) => {
      const params = {
        ...
        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/services/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: Service) => {
    const nodes = [
      {
        key: 'edit-labels',
        label: (
        ...
        const nodes = [
          <Popconfirm
            ...
            title={
              ...
              getClusterResource('Service') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      header={{ breadcrumb: {} }}
      title={intl.formatMessage({ id: 'menu.knative.Service' })}
      subTitle="Service"
    ...
    >
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          key={patchModalKey}
          kind="Service"
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchService?.metadata?.namespace}/services/${patchService?.metadata?.name}`}

          cluster={cluster}
          name={patchService?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchService?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchService?.metadata?.annotations || {}}
        />
      )}
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('Service')}:&nbsp;&nbsp;
            {patchService?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchService?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/v1/namespaces/${patchService?.metadata?.namespace}/services/${patchService?.metadata?.name}`}
          kind="Service"
          name={patchService?.metadata?.name || ''}
