...
const MessageQueueTriggerIndex: React.FC = () => {
  const colorPrimary = getColorPrimary();
  const actionRef = useRef<ActionType>(null);
  const access = useAccess();
  const { cluster, namespace } = getCurrentViewInfo();
  const [selectedRowsState, setSelectedRows] = useState<MessageQueueTrigger[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchMessageQueueTrigger, setPatchMessageQueueTrigger] = useState<MessageQueueTrigger>();
  ...
  const [dataSource, setDataSource] = useState<MessageQueueTrigger[]>([]);
  ...
  const listMessageQueueTriggers = async () => {
    setLoading(true);
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'MessageQueueTrigger';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: MessageQueueTrigger[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: MessageQueueTrigger) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/messagequeuetriggers/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: MessageQueueTrigger) => {
    const nodes = [
      {
        key: 'edit-labels',
        label: (
          <a
            onClick={() => {
              setPatchMessageQueueTrigger(record);
  ...
  const columns: ProColumns<MessageQueueTrigger>[] = [
    {
      ...
      render: (dom, entity) => {
        return (
          <>
            <a
              onClick={() => {
                setPatchMessageQueueTrigger(entity);
                setEditorResource(false);
                setResourceDrawerVisible(true);
              }}
            >
...

    {
      ...
      render: (dom, entity: MessageQueueTrigger) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            title={
              ...
              getClusterResource('MessageQueueTrigger') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      ...
      subTitle="MessageQueueTrigger"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<MessageQueueTrigger>
        key='message-queue-trigger'
        scroll={{ x: 'max-content' }}
        actionRef={actionRef}
        rowKey={(record: MessageQueueTrigger) =>
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchMessageQueueTrigger?.metadata?.namespace}/messagequeuetriggers/${patchMessageQueueTrigger?.metadata?.name}`}

          cluster={cluster}
          name={patchMessageQueueTrigger?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchMessageQueueTrigger?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchMessageQueueTrigger?.metadata?.annotations || {}}
        />
      )}
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('MessageQueueTrigger')}:&nbsp;&nbsp;
            {patchMessageQueueTrigger?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchMessageQueueTrigger?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchMessageQueueTrigger?.metadata?.namespace}/messagequeuetriggers/${patchMessageQueueTrigger?.metadata?.name}`}
          kind="MessageQueueTrigger"
          name={patchMessageQueueTrigger?.metadata?.name || ''}
          ...
          cluster={cluster}
          content={patchMessageQueueTrigger}
        />
      </Drawer>
      <AICopilot
        view='list'
...
export default MessageQueueTriggerIndex;
