...
const HTTPTriggerIndex: React.FC = () => {
  const { token } = theme.useToken();
  const colorPrimary = token.colorPrimary;
  const actionRef = useRef<ActionType>(null);
  const access = useAccess();
  const { cluster, namespace } = getCurrentViewInfo();
  const [selectedRowsState, setSelectedRows] = useState<HTTPTrigger[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchHTTPTrigger, setPatchHTTPTrigger] = useState<HTTPTrigger>();
  ...
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/httptriggers` : `apis/${resourceGroup.groupVersion}/httptriggers`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/faas/httptriggers`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/faas/httptriggers`);
  }
  const intl = useIntl();
  ...
  const [dataSource, setDataSource] = useState<HTTPTrigger[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: HTTPTrigger[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: HTTPTrigger) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/httptriggers/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: HTTPTrigger) => {
    const nodes = [
      {
        key: 'edit-labels',
        label: (
  ...
  const columns: ProColumns<HTTPTrigger>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/faas/httptriggers`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            title={
              ...
              getClusterResource('HTTPTrigger') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          key={patchModalKey}
          kind="HTTPTrigger"
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchHTTPTrigger?.metadata?.namespace}/httptriggers/${patchHTTPTrigger?.metadata?.name}`}

          cluster={cluster}
          name={patchHTTPTrigger?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchHTTPTrigger?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchHTTPTrigger?.metadata?.annotations || {}}
        />
      )}
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('HTTPTrigger')}:&nbsp;&nbsp;
            {patchHTTPTrigger?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchHTTPTrigger?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchHTTPTrigger?.metadata?.namespace}/httptriggers/${patchHTTPTrigger?.metadata?.name}`}
          kind="HTTPTrigger"
          name={patchHTTPTrigger?.metadata?.name || ''}
...
export default HTTPTriggerIndex;
