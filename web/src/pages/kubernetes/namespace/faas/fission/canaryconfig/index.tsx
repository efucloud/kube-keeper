...
const CanaryConfigIndex: React.FC = () => {
  const { token } = theme.useToken();
  const colorPrimary = token.colorPrimary;
  const actionRef = useRef<ActionType>(null);
  const access = useAccess();
  const { cluster, namespace } = getCurrentViewInfo();
  const [selectedRowsState, setSelectedRows] = useState<CanaryConfig[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchCanaryConfig, setPatchCanaryConfig] = useState<CanaryConfig>();
  ...
  const [dataSource, setDataSource] = useState<CanaryConfig[]>([]);
  ...
  const listCanaryConfigs = async () => {
    ...
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'CanaryConfig';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: CanaryConfig[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: CanaryConfig) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/canaryconfigs/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: CanaryConfig) => {
    const nodes = [
      {
        key: 'edit-labels',
        label: (
  ...
  const columns: ProColumns<CanaryConfig>[] = [
    {
...

    {
      ...
      render: (dom, entity: CanaryConfig) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            title={
              ...
              getClusterResource('CanaryConfig') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          key={patchModalKey}
          kind="CanaryConfig"
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchCanaryConfig?.metadata?.namespace}/canaryconfigs/${patchCanaryConfig?.metadata?.name}`}

          cluster={cluster}
          name={patchCanaryConfig?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchCanaryConfig?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchCanaryConfig?.metadata?.annotations || {}}
        />
      )}
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('CanaryConfig')}:&nbsp;&nbsp;
            {patchCanaryConfig?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchCanaryConfig?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchCanaryConfig?.metadata?.namespace}/canaryconfigs/${patchCanaryConfig?.metadata?.name}`}
          kind="CanaryConfig"
          name={patchCanaryConfig?.metadata?.name || ''}
...
export default CanaryConfigIndex;
