...
const TimeTriggerIndex: React.FC = () => {
  const { token } = theme.useToken();
  const colorPrimary = token.colorPrimary;
  const actionRef = useRef<ActionType>(null);
  const access = useAccess();
  const { cluster, namespace } = getCurrentViewInfo();
  const [selectedRowsState, setSelectedRows] = useState<TimeTrigger[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchTimeTrigger, setPatchTimeTrigger] = useState<TimeTrigger>();
  ...
  const [dataSource, setDataSource] = useState<TimeTrigger[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const resourceGroup = getClusterApiVersions(cluster, ['fission.io/v1'], 'TimeTrigger');
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/timetriggers` : `apis/${resourceGroup.groupVersion}/timetriggers`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/faas/timetriggers`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/faas/timetriggers`);
  }
  const intl = useIntl();
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: TimeTrigger[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: TimeTrigger) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/timetriggers/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: TimeTrigger) => {
    const nodes = [
      {
        key: 'edit-labels',
        label: (
  ...
  const columns: ProColumns<TimeTrigger>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/faas/timetriggers`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            title={
              ...
              getClusterResource('TimeTrigger') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          key={patchModalKey}
          kind="TimeTrigger"
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchTimeTrigger?.metadata?.namespace}/timetriggers/${patchTimeTrigger?.metadata?.name}`}

          cluster={cluster}
          name={patchTimeTrigger?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchTimeTrigger?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchTimeTrigger?.metadata?.annotations || {}}
        />
      )}
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('TimeTrigger')}:&nbsp;&nbsp;
            {patchTimeTrigger?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchTimeTrigger?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchTimeTrigger?.metadata?.namespace}/timetriggers/${patchTimeTrigger?.metadata?.name}`}
          kind="TimeTrigger"
          name={patchTimeTrigger?.metadata?.name || ''}
...
export default TimeTriggerIndex;
