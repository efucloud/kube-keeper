...
const KubernetesWatchTriggerIndex: React.FC = () => {
  const { token } = theme.useToken();
  const colorPrimary = token.colorPrimary;
  const actionRef = useRef<ActionType>(null);
  const access = useAccess();
  const { cluster, namespace } = getCurrentViewInfo();
  const [selectedRowsState, setSelectedRows] = useState<KubernetesWatchTrigger[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchKubernetesWatchTrigger, setPatchKubernetesWatchTrigger] = useState<KubernetesWatchTrigger>();
  ...
  const [dataSource, setDataSource] = useState<KubernetesWatchTrigger[]>([]);
  ...
  const listKubernetesWatchTriggers = async () => {
    setLoading(true);
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'KubernetesWatchTrigger';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: KubernetesWatchTrigger[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: KubernetesWatchTrigger) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/kuberneteswatchtriggers/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: KubernetesWatchTrigger) => {
    const nodes = [
      {
        key: 'edit-labels',
        label: (
          <a
            onClick={() => {
              setPatchKubernetesWatchTrigger(record);
  ...
  const columns: ProColumns<KubernetesWatchTrigger>[] = [
    {
      ...
      render: (dom, entity) => {
        return (
          <>
            <a
              onClick={() => {
                setPatchKubernetesWatchTrigger(entity);
                setEditorResource(false);
                setResourceDrawerVisible(true);
              }}
            >
...

    {
      ...
      render: (dom, entity: KubernetesWatchTrigger) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.environment.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('KubernetesWatchTrigger') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      header={{ breadcrumb: {} }}
      title={intl.formatMessage({ id: 'menu.faas.Fission.environment' })}
      subTitle="KubernetesWatchTrigger"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<KubernetesWatchTrigger>
        key='kubernetes-watch-trigger'
        scroll={{ x: 'max-content' }}
        actionRef={actionRef}
        rowKey={(record: KubernetesWatchTrigger) =>
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchKubernetesWatchTrigger?.metadata?.namespace}/kuberneteswatchtriggers/${patchKubernetesWatchTrigger?.metadata?.name}`}

          cluster={cluster}
          name={patchKubernetesWatchTrigger?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchKubernetesWatchTrigger?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchKubernetesWatchTrigger?.metadata?.annotations || {}}
        />
      )}
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('KubernetesWatchTrigger')}:&nbsp;&nbsp;
            {patchKubernetesWatchTrigger?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchKubernetesWatchTrigger?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/fission.io/v1/namespaces/${patchKubernetesWatchTrigger?.metadata?.namespace}/kuberneteswatchtriggers/${patchKubernetesWatchTrigger?.metadata?.name}`}
          kind="KubernetesWatchTrigger"
          name={patchKubernetesWatchTrigger?.metadata?.name || ''}

          cluster={cluster}
          content={patchKubernetesWatchTrigger}
        />
      </Drawer>
      <AICopilot
        view='list'
...
export default KubernetesWatchTriggerIndex;
