...
const FunctionIndex: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Function[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchFunction, setPatchFunction] = useState<Function>();
  ...
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/functions` : `apis/${resourceGroup.groupVersion}/functions`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/faas/functions`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/faas/functions`);
  }
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Function[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Function) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/functions/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: Function) => {
    const nodes = [
      {
        key: 'edit-labels',
        label: (
  ...
  const columns: ProColumns<Function>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/faas/functions`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            title={
              ...
              getClusterResource('Function') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          key={patchModalKey}
          kind="Function"
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchFunction?.metadata?.namespace}/functions/${patchFunction?.metadata?.name}`}

          cluster={cluster}
          name={patchFunction?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchFunction?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchFunction?.metadata?.annotations || {}}
        />
      )}
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('Function')}:&nbsp;&nbsp;
            {patchFunction?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchFunction?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchFunction?.metadata?.namespace}/functions/${patchFunction?.metadata?.name}`}
          kind="Function"
          name={patchFunction?.metadata?.name || ''}
