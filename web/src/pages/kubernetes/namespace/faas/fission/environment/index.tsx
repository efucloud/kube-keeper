...
import { Button, Drawer, Dropdown, Modal, message, Popconfirm, Popover, Space, Tag, Typography, Empty, Select, Divider, theme } from 'antd';
...
const EnvironmentIndex: React.FC = () => {
  const { token } = theme.useToken();
  const colorPrimary = token.colorPrimary;
  const actionRef = useRef<ActionType>(null);
  const access = useAccess();
  const { cluster, namespace } = getCurrentViewInfo();
  const [selectedRowsState, setSelectedRows] = useState<Environment[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchEnvironment, setPatchEnvironment] = useState<Environment>();
  ...
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/environments` : `apis/${resourceGroup.groupVersion}/environments`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/faas/environments`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/faas/environments`);
  }
  const intl = useIntl();
  ...
  const [dataSource, setDataSource] = useState<Environment[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Environment[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Environment) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/environments/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: Environment) => {
    const nodes = [
      {
        key: 'edit-labels',
        label: (
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/faas/environments`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            title={
              ...
              getClusterResource('Environment') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          key={patchModalKey}
          kind="Environment"
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchEnvironment?.metadata?.namespace}/environments/${patchEnvironment?.metadata?.name}`}

          cluster={cluster}
          name={patchEnvironment?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchEnvironment?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchEnvironment?.metadata?.annotations || {}}
        />
      )}
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('Environment')}:&nbsp;&nbsp;
            {patchEnvironment?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchEnvironment?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchEnvironment?.metadata?.namespace}/environments/${patchEnvironment?.metadata?.name}`}
          kind="Environment"
          name={patchEnvironment?.metadata?.name || ''}
