...
const PackageIndex: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Package[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchPackage, setPatchPackage] = useState<Package>();
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Package[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Package) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/packages/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: Package) => {
    const nodes = [
      {
        key: 'edit-labels',
        label: (
        ...
        const nodes = [
          <Popconfirm
            ...
            title={
              ...
              getClusterResource('Package') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          key={patchModalKey}
          kind="Package"
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchPackage?.metadata?.namespace}/packages/${patchPackage?.metadata?.name}`}

          cluster={cluster}
          name={patchPackage?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchPackage?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchPackage?.metadata?.annotations || {}}
        />
      )}
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('Package')}:&nbsp;&nbsp;
            {patchPackage?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchPackage?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchPackage?.metadata?.namespace}/packages/${patchPackage?.metadata?.name}`}
          kind="Package"
          name={patchPackage?.metadata?.name || ''}
