...
import { Button, Divider, Dropdown, Modal, message, Popconfirm, Popover, Select, Space, Tag, Typography, Empty, theme } from 'antd';
import React, { useEffect, useRef, useState } from 'react';
const { Text } = Typography;
import dayjs from 'dayjs';
import { IResourceQuota, type ResourceQuotaList } from 'kubernetes-models/v1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<IResourceQuota[]>([]);
  const [dataSource, setDataSource] = useState<IResourceQuota[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const formRef = useRef<ProFormInstance>(undefined);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchResourceQuota, setPatchResourceQuota] = useState<IResourceQuota>();
  ...
  const address = namespace ? `api/v1/namespaces/${namespace}/resourcequotas` : `api/v1/resourcequotas`;
  ...
  const listResourceQuotas = async () => {
    setLoading(true);
    try {
      ...
      const data = (await clusterGetProxy(params)) as ResourceQuotaList;
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'ResourceQuota';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: IResourceQuota[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: IResourceQuota) => {
      const params = {

        cluster,
        address: `api/v1/namespaces/${entity.metadata?.namespace}/resourcequotas/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: IResourceQuota) => {
    const nodes = [];
  ...
  const columns: ProColumns<IResourceQuota>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/config/resourcequotas`),
                );
              }}
            >
      ...
      render: (dom, entity) => {
        return (
          <>
            <a
              onClick={() => {
                window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/config/resourcequotas/${entity?.metadata?.name}/update`);
              }}
            >
...

    {
      ...
      render: (dom, entity: IResourceQuota) => {
      ...
      render: (_, record) => {
        const nodes = [
          <a
            key="edit"
            onClick={() => {
              window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${record?.metadata?.namespace}/config/resourcequotas/${record?.metadata?.name}/update`);
            }}
          >
          ...
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.resourcequota.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('IResourceQuota') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      header={{ breadcrumb: {} }}
      title={intl.formatMessage({ id: 'menu.config.resourcequota' })}
      subTitle="ResourceQuota"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<IResourceQuota>
        key='resource-quota'
        actionRef={actionRef}
        formRef={formRef}
        scroll={{ x: 'max-content' }}
        rowKey={(record: IResourceQuota) =>
        ...
        options={{ reload: () => { listResourceQuotas(); } }}
        locale={{
              ...
              <Button
                ...
                onClick={() => {
                  window.location.href = namespace
                    ? toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/config/resourcequotas/create/text`)
                    : toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/config/resourcequotas/create/text`);
                }}
              >
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          key={patchModalKey}
          kind="ResourceQuota"
          address={`api/v1/namespaces/${patchResourceQuota?.metadata?.namespace}/resourcequotas/${patchResourceQuota?.metadata?.name}`}

          cluster={cluster}
          name={patchResourceQuota?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchResourceQuota?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchResourceQuota?.metadata?.annotations || {}}
        />
      )}
