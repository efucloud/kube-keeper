import { CloseCircleOutlined, EditOutlined, InsertRowBelowOutlined, MoreOutlined, UnorderedListOutlined } from '@ant-design/icons';
...
import type { ILimitRange, LimitRangeList } from 'kubernetes-models/v1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<ILimitRange[]>([]);
  const [dataSource, setDataSource] = useState<ILimitRange[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const formRef = useRef<ProFormInstance>(undefined);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchLabelVisible, setPatchLabelVisible] = useState<boolean>(false);
  const [patchLimitRange, setPatchLimitRange] = useState<ILimitRange>();
  ...
  const address = namespace ? `api/v1/namespaces/${namespace}/limitranges` : `api/v1/limitranges`;
  ...
  const listLimitRanges = async () => {
    ...
    try {
      ...
      const data = (await clusterGetProxy(params)) as LimitRangeList;
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: ILimitRange[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: ILimitRange) => {
      const params = {

        cluster,
        address: `api/v1/namespaces/${entity.metadata?.namespace}/limitranges/${entity.metadata?.name}`,
      };
  ...
  const moreItems = (record: ILimitRange) => {
    const nodes = [];
  ...
  const columns: ProColumns<ILimitRange>[] = [
    {
        ...
        const nodes = [
          ...
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.LimitRange.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('LimitRange') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      header={{ breadcrumb: {} }}
      title={intl.formatMessage({ id: 'menu.config.limitrange' })}
      subTitle="LimitRange"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<ILimitRange>
        key='limit-range'
        actionRef={actionRef}
        formRef={formRef}
        scroll={{ x: 'max-content' }}
        rowKey={(record: ILimitRange) =>
      ...
      {patchLabelVisible && (
        <PatchLabels
          ...
          key={patchModalKey}
          kind="LimitRange"
          address={`api/v1/namespaces/${patchLimitRange?.metadata?.namespace}/limitranges/${patchLimitRange?.metadata?.name}`}

          cluster={cluster}
          name={patchLimitRange?.metadata?.name || ''}
          visible={patchLabelVisible}
          labels={patchLimitRange?.metadata?.labels || {}}
        />
      )}
      {patchAnnotationsVisible && (
        <PatchLabels
          ...
          labels={patchLimitRange?.metadata?.annotations || {}}
        />
      )}
