...
const DetailPage: React.FC = () => {
  ...
  const [info, setInfo] = useState<IVirtualMachineSnapshot>();
  const [resolvedNamespace, setResolvedNamespace] = useState(namespace || '');
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachinesnapshots', resolvedNamespace);
  ...
  useEffect(() => {
    const load = async () => {
      if (!name) return;
      const res = await resolveKubeVirtResourceByName<IVirtualMachineSnapshot>({
        cluster,
    ...
    >
      <Card>
        <Descriptions column={2}>
          ...
          <Descriptions.Item label={intl.formatMessage({ id: 'cluster.resource.kind' })}>{formatResourceKind(intl, info?.kind || 'VirtualMachineSnapshot')}</Descriptions.Item>
