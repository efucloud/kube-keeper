...
import type { IVirtualMachineSnapshot } from '@/k8s-models/kubevirt/snapshot.kubevirt.io/v1beta1';
...
const IndexPage: React.FC = () => {
  const intl = useIntl();
  const resourceLabel = formatResourceKind(intl, 'VirtualMachineSnapshot');
  const actionRef = useRef<ActionType>(null);
  const { cluster, namespace } = getCurrentViewInfo();
  const resourceGroup = useMemo(
    () => getClusterApiVersions(cluster, ['snapshot.kubevirt.io/v1beta1', 'snapshot.kubevirt.io/v1alpha1'], 'VirtualMachineSnapshot'),
    [cluster],
  );
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachinesnapshots', namespace);
  const baseAddress = buildKubeVirtBaseAddress(cluster, 'virtualmachinesnapshots', namespace);
  const [dataSource, setDataSource] = useState<IVirtualMachineSnapshot[]>([]);
  const [loading, setLoading] = useState(false);

  const getStatusText = (resource?: IVirtualMachineSnapshot) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IVirtualMachineSnapshot[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'VirtualMachineSnapshot',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IVirtualMachineSnapshot>[] = [
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              await clusterDeleteProxy({ cluster, address: `${buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachinesnapshots', namespace || record.metadata?.namespace || '')}/${record.metadata?.name || ''}` });
  ...
  return (
    <PageContainer header={{ breadcrumb: {} }} title={resourceLabel} subTitle={resourceLabel}>
      <ProTable<IVirtualMachineSnapshot>
        actionRef={actionRef}
