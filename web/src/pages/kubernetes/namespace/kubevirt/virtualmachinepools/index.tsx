...
const IndexPage: React.FC = () => {
  ...
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachinepools', namespace);
  const baseAddress = buildKubeVirtBaseAddress(cluster, 'virtualmachinepools', namespace);
  const [dataSource, setDataSource] = useState<IVirtualMachinePool[]>([]);
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IVirtualMachinePool[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'VirtualMachinePool',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IVirtualMachinePool>[] = [
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              await clusterDeleteProxy({ cluster, address: `${buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachinepools', namespace || record.metadata?.namespace || '')}/${record.metadata?.name || ''}` });
