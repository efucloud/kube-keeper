...
const IndexPage: React.FC = () => {
  ...
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachineinstances', namespace);
  const baseAddress = buildKubeVirtBaseAddress(cluster, 'virtualmachineinstances', namespace);
  const [dataSource, setDataSource] = useState<IVirtualMachineInstance[]>([]);
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IVirtualMachineInstance[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'VirtualMachineInstance',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IVirtualMachineInstance>[] = [
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              await clusterDeleteProxy({ cluster, address: `${buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachineinstances', namespace || record.metadata?.namespace || '')}/${record.metadata?.name || ''}` });
  ...
  return (
    <PageContainer header={{ breadcrumb: {} }} title={resourceLabel} subTitle={resourceLabel}>
      <ProTable<IVirtualMachineInstance>
        actionRef={actionRef}
