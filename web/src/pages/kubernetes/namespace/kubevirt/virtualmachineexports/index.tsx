...
const IndexPage: React.FC = () => {
  ...
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachineexports', namespace);
  const baseAddress = buildKubeVirtBaseAddress(cluster, 'virtualmachineexports', namespace);
  const [dataSource, setDataSource] = useState<IVirtualMachineExport[]>([]);
  const [loading, setLoading] = useState(false);

  const getStatusText = (resource?: IVirtualMachineExport) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IVirtualMachineExport[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'VirtualMachineExport',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IVirtualMachineExport>[] = [
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              await clusterDeleteProxy({ cluster, address: `${buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachineexports', namespace || record.metadata?.namespace || '')}/${record.metadata?.name || ''}` });
  ...
  return (
    <PageContainer header={{ breadcrumb: {} }} title={resourceLabel} subTitle={resourceLabel}>
      <ProTable<IVirtualMachineExport>
        actionRef={actionRef}
