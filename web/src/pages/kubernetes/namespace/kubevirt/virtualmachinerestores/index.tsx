...
import type { IVirtualMachineRestore } from '@/k8s-models/kubevirt/snapshot.kubevirt.io/v1beta1';
...
const IndexPage: React.FC = () => {
  const intl = useIntl();
  const resourceLabel = formatResourceKind(intl, 'VirtualMachineRestore');
  const actionRef = useRef<ActionType>(null);
  const { cluster, namespace } = getCurrentViewInfo();
  const resourceGroup = useMemo(
    () => getClusterApiVersions(cluster, ['snapshot.kubevirt.io/v1beta1', 'snapshot.kubevirt.io/v1alpha1'], 'VirtualMachineRestore'),
    [cluster],
  );
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachinerestores', namespace);
  const baseAddress = buildKubeVirtBaseAddress(cluster, 'virtualmachinerestores', namespace);
  const [dataSource, setDataSource] = useState<IVirtualMachineRestore[]>([]);
  const [loading, setLoading] = useState(false);

  const getStatusText = (resource?: IVirtualMachineRestore) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IVirtualMachineRestore[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'VirtualMachineRestore',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IVirtualMachineRestore>[] = [
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              await clusterDeleteProxy({ cluster, address: `${buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachinerestores', namespace || record.metadata?.namespace || '')}/${record.metadata?.name || ''}` });
  ...
  return (
    <PageContainer header={{ breadcrumb: {} }} title={resourceLabel} subTitle={resourceLabel}>
      <ProTable<IVirtualMachineRestore>
        actionRef={actionRef}
