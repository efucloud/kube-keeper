...
const DetailPage: React.FC = () => {
  ...
  const [info, setInfo] = useState<IVirtualMachineRestore>();
  const [resolvedNamespace, setResolvedNamespace] = useState(namespace || '');
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachinerestores', resolvedNamespace);
  ...
  useEffect(() => {
    const load = async () => {
      if (!name) return;
      const res = await resolveKubeVirtResourceByName<IVirtualMachineRestore>({
        cluster,
    ...
    >
      <Card>
        <Descriptions column={2}>
          ...
          <Descriptions.Item label={intl.formatMessage({ id: 'cluster.resource.kind' })}>{formatResourceKind(intl, info?.kind || 'VirtualMachineRestore')}</Descriptions.Item>
