...
const IndexPage: React.FC = () => {
  ...
  const [dataSource, setDataSource] = useState<IKubeVirt[]>([]);
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IKubeVirt[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'KubeVirt',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IKubeVirt>[] = [
    {
    ...
    },
    {
      ...
      search: false,
      render: (_, entity) =>
        entity.metadata?.creationTimestamp ? dayjs(entity.metadata.creationTimestamp).format('YYYY-MM-DD HH:mm:ss') : '-',
    },
    {
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              await clusterDeleteProxy({
                cluster,
                address: `${baseApi}/${record.metadata?.name || ''}`,
              });
  ...
  return (
    <PageContainer header={{ breadcrumb: {} }} title={resourceLabel} subTitle={resourceLabel}>
      <ProTable<IKubeVirt>
        actionRef={actionRef}
        rowKey={(record) => `${record.metadata?.namespace || namespace}-${record.metadata?.name || ''}-${record.metadata?.resourceVersion || 'latest'}`}
        loading={loading}
