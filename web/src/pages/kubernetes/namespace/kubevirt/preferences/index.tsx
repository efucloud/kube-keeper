...
const IndexPage: React.FC = () => {
  ...
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachinepreferences', namespace);
  const baseAddress = buildKubeVirtBaseAddress(cluster, 'preferences', namespace);
  const [dataSource, setDataSource] = useState<IVirtualMachinePreference[]>([]);
  const [loading, setLoading] = useState(false);

  const getStatusText = (resource?: IVirtualMachinePreference) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IVirtualMachinePreference[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'VirtualMachinePreference',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IVirtualMachinePreference>[] = [
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              await clusterDeleteProxy({ cluster, address: `${buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachinepreferences', namespace || record.metadata?.namespace || '')}/${record.metadata?.name || ''}` });
  ...
  return (
    <PageContainer header={{ breadcrumb: {} }} title={resourceLabel} subTitle={resourceLabel}>
      <ProTable<IVirtualMachinePreference>
        actionRef={actionRef}
