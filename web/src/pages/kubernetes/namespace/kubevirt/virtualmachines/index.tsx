...
const IndexPage: React.FC = () => {
  ...
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachines', namespace);
  const baseAddress = buildKubeVirtBaseAddress(cluster, 'virtualmachines', namespace);
  const [dataSource, setDataSource] = useState<IVirtualMachine[]>([]);
  const [loading, setLoading] = useState(false);

  const getStatusText = (resource?: IVirtualMachine) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IVirtualMachine[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'VirtualMachine',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IVirtualMachine>[] = [
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              await clusterDeleteProxy({ cluster, address: `${buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'virtualmachines', namespace || record.metadata?.namespace || '')}/${record.metadata?.name || ''}` });
