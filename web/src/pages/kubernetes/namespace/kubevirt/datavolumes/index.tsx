...
import type { IDataVolume } from '@/k8s-models/kubevirt/cdi.kubevirt.io/v1beta1';
...
const IndexPage: React.FC = () => {
  const intl = useIntl();
  const resourceLabel = formatResourceKind(intl, 'DataVolume');
  const actionRef = useRef<ActionType>(null);
  const { cluster, namespace } = getCurrentViewInfo();
  const resourceGroup = useMemo(
    () => getClusterApiVersions(cluster, ['cdi.kubevirt.io/v1beta1'], 'DataVolume'),
    [cluster],
  );
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'datavolumes', namespace);
  const baseAddress = buildKubeVirtBaseAddress(cluster, 'datavolumes', namespace);
  const [dataSource, setDataSource] = useState<IDataVolume[]>([]);
  const [loading, setLoading] = useState(false);

  const getStatusText = (resource?: IDataVolume) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDataSource(((data.items || []) as IDataVolume[]).map((item) => ({
        ...item,
        apiVersion: item.apiVersion || resourceGroup.groupVersion,
        kind: item.kind || 'DataVolume',
      })));
    } finally {
      setLoading(false);
    }
  ...
  const columns: ProColumns<IDataVolume>[] = [
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm
            ...
            onConfirm={async () => {
              await clusterDeleteProxy({ cluster, address: `${buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'datavolumes', namespace || record.metadata?.namespace || '')}/${record.metadata?.name || ''}` });
