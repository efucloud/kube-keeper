...
const DetailPage: React.FC = () => {
  ...
  const [info, setInfo] = useState<IDataVolume>();
  const [resolvedNamespace, setResolvedNamespace] = useState(namespace || '');
  const baseApi = buildKubeVirtCollectionApi(resourceGroup.groupVersion, 'datavolumes', resolvedNamespace);
  ...
  useEffect(() => {
    const load = async () => {
      if (!name) return;
      const res = await resolveKubeVirtResourceByName<IDataVolume>({
        cluster,
    ...
    >
      <Card>
        <Descriptions column={2}>
          ...
          <Descriptions.Item label={intl.formatMessage({ id: 'cluster.resource.kind' })}>{formatResourceKind(intl, info?.kind || 'DataVolume')}</Descriptions.Item>
