...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<IApplicationSet[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchAppProject, setPatchAppProject] = useState<IApplicationSet>();
  ...
  const resourceGroup = getClusterApiVersions(cluster, ['argoproj.io/v1alpha1'], 'ApplicationSet');
  const address = namespace ? `apis/${resourceGroup?.groupVersion}/namespaces/${namespace}/applicationsets` : `apis/${resourceGroup?.groupVersion}/applicationsets`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/devops/cicd/argocd/applicationsets`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/devops/cicd/argocd/applicationsets`);
  }
  const intl = useIntl();
  ...
  const [dataSource, setDataSource] = useState<IApplicationSet[]>([]);
  ...
  const listAppProjects = async () => {
    ...
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'ApplicationSet';
      }
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: IApplicationSet[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: IApplicationSet) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup?.groupVersion}/namespaces/${entity.metadata?.namespace}/applicationsets/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<IApplicationSet>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/devops/cicd/argocd/applicationsets`),
                );
              }}
            >
...

    {
      ...
      render: (dom, entity: IApplicationSet) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.argocd.ApplicationSet.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('ApplicationSet') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    ...
    >
      <ProTable<IApplicationSet>
        key='application-set'
        scroll={{ x: 'max-content' }}
        actionRef={actionRef}
        rowKey={(record: IApplicationSet) =>
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('ApplicationSet')}:&nbsp;&nbsp;
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup?.groupVersion}/namespaces/${patchAppProject?.metadata?.namespace}/applicationsets/${patchAppProject?.metadata?.name}`}
          kind="ApplicationSet"
