...
import { Button, Drawer, Modal, message, Popconfirm, Popover, Space, Tag, Typography, Empty, Select, theme } from 'antd';
...
import { CncfArgoCD, CncfVolcano } from '@/utils/cncf';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<IAppProject[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchAppProject, setPatchAppProject] = useState<IAppProject>();
  ...
  const [dataSource, setDataSource] = useState<IAppProject[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const resourceGroup = getClusterApiVersions(cluster, ['argoproj.io/v1alpha1'], 'AppProject');
  const address = namespace ? `apis/${resourceGroup?.groupVersion}/namespaces/${namespace}/appprojects` : `apis/${resourceGroup?.groupVersion}/appprojects`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/devops/cicd/argocd/appprojects`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/devops/cicd/argocd/appprojects`);
  }
  const intl = useIntl();
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: IAppProject[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: IAppProject) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup?.groupVersion}/namespaces/${entity.metadata?.namespace}/appprojects/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<IAppProject>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/devops/cicd/argocd/appprojects`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.argocd.AppProject.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('AppProject') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    ...
    >
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('AppProject')}:&nbsp;&nbsp;
            {patchAppProject?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchAppProject?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup?.groupVersion}/namespaces/${patchAppProject?.metadata?.namespace}/appprojects/${patchAppProject?.metadata?.name}`}
          kind="AppProject"
          name={patchAppProject?.metadata?.name || ''}

          cluster={cluster}
          content={patchAppProject}
        />
      </Drawer>
      <AICopilot
        view='list'
