...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<IApplication[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchAppProject, setPatchAppProject] = useState<IApplication>();
  ...
  const resourceGroup = getClusterApiVersions(cluster, ['argoproj.io/v1alpha1'], 'Application');
  const address = namespace ? `apis/${resourceGroup?.groupVersion}/namespaces/${namespace}/applications` : `apis/${resourceGroup?.groupVersion}/applications`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/devops/cicd/argocd/applications`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/devops/cicd/argocd/applications`);
  }
  const intl = useIntl();
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: IApplication[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: IApplication) => {
      const params = { cluster, address: `apis/${resourceGroup?.groupVersion}/namespaces/${entity.metadata?.namespace}/applications/${entity.metadata?.name}` };
  ...
  const columns: ProColumns<IApplication>[] = [
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/devops/cicd/argocd/applications`),
                );
              }}
            >
...

    {
      ...
      render: (dom, entity: IApplication) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            title={
              ...
              getClusterResource('IApplication') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    ...
    >
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('Application')}:&nbsp;&nbsp;
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup?.groupVersion}/namespaces/${patchAppProject?.metadata?.namespace}/applications/${patchAppProject?.metadata?.name}`}
          kind="Application"
