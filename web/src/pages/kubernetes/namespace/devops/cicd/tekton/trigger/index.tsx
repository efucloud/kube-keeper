...
import type { Trigger } from '@/k8s-models/tekton/triggers/triggers.tekton.dev/v1beta1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Trigger[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchTrigger, setPatchTrigger] = useState<Trigger>();
  const formRef = useRef<ProFormInstance>(undefined);
  const [dataSource, setDataSource] = useState<Trigger[]>([]);
  ...
  const address = namespace ? `apis/${resourceGroup?.groupVersion}/namespaces/${namespace}/triggers` : `apis/${resourceGroup?.groupVersion}/triggers`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/devops/cicd/tekton/triggers`);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Trigger[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Trigger) => {
      const params = { cluster, address: `apis/${resourceGroup?.groupVersion}/namespaces/${entity.metadata?.namespace}/triggers/${entity.metadata?.name}` };
  ...
  const columns: ProColumns<Trigger>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/devops/cicd/tekton/triggers`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.tekton.Trigger.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('Trigger') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      header={{ breadcrumb: {} }}
      title={intl.formatMessage({ id: 'menu.DevOps.Tekton.trigger' })}
      subTitle="Trigger"
    ...
    >
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('Trigger')}:&nbsp;&nbsp;
            {patchTrigger?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchTrigger?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup?.groupVersion}/namespaces/${patchTrigger?.metadata?.namespace}/triggers/${patchTrigger?.metadata?.name}`}
          kind="Trigger"
          name={patchTrigger?.metadata?.name || ''}

          cluster={cluster}
          content={patchTrigger}
        />
      </Drawer>
      <AICopilot
        view='list'
