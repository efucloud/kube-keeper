...
import { Button, Divider, Drawer, Modal, message, Popconfirm, Popover, Space, Tag, Typography, Select, Empty, theme } from 'antd';
import React, { useEffect, useRef, useState } from 'react';
const { Text } = Typography;
import dayjs from 'dayjs';
import type { IntlShape } from 'react-intl';
import { TriggerTemplate } from '@/k8s-models/tekton/triggers/triggers.tekton.dev/v1beta1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<TriggerTemplate[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchTriggerTemplate, setPatchTriggerTemplate] = useState<TriggerTemplate>();
  const formRef = useRef<ProFormInstance>(undefined);
  const [dataSource, setDataSource] = useState<TriggerTemplate[]>([]);
  ...
  const resourceGroup = getClusterApiVersions(cluster, ['triggers.tekton.dev/v1beta1'], 'TriggerTemplate');
  const address = namespace ? `apis/${resourceGroup?.groupVersion}/namespaces/${namespace}/triggers` : `apis/${resourceGroup?.groupVersion}/triggertemplates`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/devops/cicd/tekton/triggertemplates`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/devops/cicd/tekton/triggertemplates`);
  }
  const intl = useIntl();
  ...
  const listTriggerTemplates = async () => {
    setLoading(true);
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'TriggerTemplate';
      }
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: TriggerTemplate[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: TriggerTemplate) => {
      const params = { cluster, address: `apis/${resourceGroup?.groupVersion}/namespaces/${entity.metadata?.namespace}/triggertemplates/${entity.metadata?.name}` };
  ...
  const columns: ProColumns<TriggerTemplate>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/devops/cicd/tekton/triggertemplates`),
                );
              }}
            >
      ...
      render: (dom, entity) => {
        return (
          <>
            <a
              onClick={() => {
                setPatchTriggerTemplate(entity);
...

    {
      ...
      render: (dom, entity: TriggerTemplate) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.tekton.TriggerTemplate.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('TriggerTemplate') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
        ...
        if (!record?.metadata?.ownerReferences) {
          nodes.unshift(
            <a
              key="edit"
              onClick={() => {
                setPatchTriggerTemplate(record);
                setResourceDrawerVisible(true);
      ...
      <Drawer
        ...
        title={
          <>
            ...
            {patchTriggerTemplate?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchTriggerTemplate?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup?.groupVersion}/namespaces/${patchTriggerTemplate?.metadata?.namespace}/triggertemplates/${patchTriggerTemplate?.metadata?.name}`}
          kind="TriggerTemplate"
          name={patchTriggerTemplate?.metadata?.name || ''}

          cluster={cluster}
          content={patchTriggerTemplate}
        />
      </Drawer>
      <AICopilot
        view='list'
