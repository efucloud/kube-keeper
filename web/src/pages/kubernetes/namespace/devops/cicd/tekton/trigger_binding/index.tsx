...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<TriggerBinding[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchTriggerBinding, setPatchTriggerBinding] =
    useState<TriggerBinding>();
  const formRef = useRef<ProFormInstance>(undefined);
  const [dataSource, setDataSource] = useState<TriggerBinding[]>([]);
  ...
  const resourceGroup = getClusterApiVersions(cluster, ['triggers.tekton.dev/v1beta1'], 'TriggerBinding');
  const address = namespace ? `apis/${resourceGroup?.groupVersion}/namespaces/${namespace}/triggers` : `apis/${resourceGroup?.groupVersion}/triggerbindings`;
  ...
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/devops/cicd/tekton/triggerbindings`);
  }
  const intl = useIntl();
  ...
  const listTriggerBindings = async () => {
    setLoading(true);
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'TriggerBinding';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: TriggerBinding[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: TriggerBinding) => {
      const params = { cluster, address: `apis/${resourceGroup?.groupVersion}/namespaces/${entity.metadata?.namespace}/triggerbindings/${entity.metadata?.name}` };
  ...
  const columns: ProColumns<TriggerBinding>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/devops/cicd/tekton/triggerbindings`),
                );
              }}
            >
...

    {
      ...
      render: (dom, entity: TriggerBinding) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.tekton.TriggerBinding.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('TriggerBinding') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      header={{ breadcrumb: {} }}
      title={intl.formatMessage({ id: 'menu.DevOps.Tekton.triggerBinding' })}
      subTitle="TriggerBinding"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<TriggerBinding>
        key='trigger-binding'
        actionRef={actionRef}
        formRef={formRef}
        scroll={{ x: 'max-content' }}
        rowKey={(record: TriggerBinding) =>
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('TriggerBinding')}:&nbsp;&nbsp;
            {patchTriggerBinding?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchTriggerBinding?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup?.groupVersion}/namespaces/${patchTriggerBinding?.metadata?.namespace}/triggerbindings/${patchTriggerBinding?.metadata?.name}`}
          kind="TriggerBinding"
          name={patchTriggerBinding?.metadata?.name || ''}

          cluster={cluster}
          content={patchTriggerBinding}
        />
      </Drawer>
      <AICopilot
        view='list'
