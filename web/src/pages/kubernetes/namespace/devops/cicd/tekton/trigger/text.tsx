...
const YamOrJsonForm: FC<Record<string, any>> = () => {
  ...
  const [info, setInfo] = useState<Trigger>();
  ...
  const BaseApi = `apis/${resourceGroup?.groupVersion}/namespaces/${namespace}/triggers`;
  ...
  const getInfo = async () => {
    ...
    const res = (await clusterGetProxy(params)) as Trigger;
    setInfo(res);
  };
  useEffect(() => {
    if (name && name !== '' && name !== '-') {
