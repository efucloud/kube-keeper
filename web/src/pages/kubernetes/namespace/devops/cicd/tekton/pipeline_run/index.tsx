import { CheckCircleFilled, CloseCircleFilled, CloseCircleOutlined, DeleteOutlined, ExclamationCircleOutlined, FileOutlined, InsertRowBelowOutlined, RedoOutlined, QuestionCircleOutlined, PlayCircleOutlined } from '@ant-design/icons';
...
import type { PipelineRun } from '@/k8s-models/tekton/pipeline/tekton.dev/v1';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<PipelineRun[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchPipelineRun, setPatchPipelineRun] = useState<PipelineRun>();
  const [dataSource, setDataSource] = useState<PipelineRun[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: PipelineRun[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: PipelineRun) => {
      const params = { cluster, address: `apis/${resourceGroup?.groupVersion}/namespaces/${entity.metadata?.namespace}/pipelineruns/${entity.metadata?.name}` };
  ...
  const reDeploy = async (pipelineRun: PipelineRun) => {
    await clusterDeleteProxy({
      ...
      cluster,
      address: `apis/${resourceGroup?.groupVersion}/namespaces/${pipelineRun.metadata?.namespace}/pipelineruns/${pipelineRun.metadata?.name}`,
    });
    ...
    await clusterPostProxy(
      {
        ...
        cluster,
        address: `apis/${resourceGroup?.groupVersion}/namespaces/${requestData.metadata?.namespace}/pipelineruns`,
      },
      requestData,
    );
    message.success(intl.formatMessage({ id: 'cluster.pages.operation.success' }));
  ...
  const columns: ProColumns<PipelineRun>[] = [
    {
    ...
    },
    {
      ...
      key: 'pipeline',
      search: false,
      render: (dom, entity: PipelineRun) => {
        return (
          <>
            {entity.spec?.pipelineRef?.name ||
              entity.status?.pipelineSpec?.displayName ||
              ''}
          </>
        );
      },
    ...
    {
      title: intl.formatMessage({ id: 'model.task.number' }),
      dataIndex: 'task.number',
      search: false,
      render: (dom, entity) => {
        return (
          <>
            <>{entity?.status?.childReferences?.length}</>
          </>
        );
      },
    },
        ...
        if (status === 'Running') {
          return (
            <>
              ...
              />
              &nbsp;&nbsp;
              <FormattedMessage id={`model.${status}`} />{message && <> <Tooltip color={colorPrimary} title={message}><QuestionCircleOutlined /></Tooltip></>}
            </>
          );
        }
        if (status === 'Succeeded') {
          return (
            <>
              ...
              &nbsp;&nbsp;
              <FormattedMessage id={`model.Succeeded`} />{message && <> <Tooltip color={colorPrimary} title={message}><QuestionCircleOutlined /></Tooltip></>}
            </>
          );
          ...
          return (
            <>
              ...
              &nbsp;&nbsp;
              <FormattedMessage id={`model.Failed`} />{message && <> <Tooltip color={colorPrimary} title={message}><QuestionCircleOutlined /></Tooltip></>}
            </>
          );
          ...
          return (
            <>
              <CheckCircleFilled style={{ color: '#52c41a' }} />
              &nbsp;&nbsp;
              <FormattedMessage id={`model.Completed`} />{message && <> <Tooltip color={colorPrimary} title={message}><QuestionCircleOutlined /></Tooltip></>}
            </>
          );
        } else {
          return (
    ...
    },
    {
      ...
      render: (dom, entity: PipelineRun) => {
        ...
        return (
          <div>
            {Object.keys(entity?.metadata?.labels || {})?.map((key: string) => (
              <>
                <Tag style={{ border: 0 }} key={key}>
    ...
    },
    {
      ...
      render: (_, record) => {
        const nodes = [
          ...
          >
            <PlayCircleOutlined style={{ color: colorPrimary }} />
          </Popconfirm>,
          ...
          <Popconfirm
            ...
            title={
              ...
              getClusterResource('PipelineRun') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
        ...
        if (!record?.metadata?.ownerReferences) {
          nodes.unshift(
            ...
            >
              <Tooltip
                color={colorPrimary}
                title={<FormattedMessage id="cluster.view.yaml" />}
              >
                <FileOutlined style={{ color: colorPrimary }} />
              </Tooltip>
            </a>,
          );
        }
  ...
  return (
    <PageContainer
      header={{ breadcrumb: {} }}
      title={intl.formatMessage({ id: 'menu.DevOps.Tekton.pipelineRun' })}
      subTitle="PipelineRun"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<PipelineRun>
        key='pipeline-run'
        scroll={{ x: 'max-content' }}
        actionRef={actionRef}
        formRef={formRef}
        rowKey={(record: PipelineRun) =>
            ...
            onClick={() => {
              Modal.confirm({
                ...
                content: intl.formatMessage({
                  id: 'cluster.resource.tekton.PipelineRun.delete.description',
                }),
                okText: intl.formatMessage({ id: 'pages.operation.confirm' }),
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('PipelineRun')}:&nbsp;&nbsp;
            {patchPipelineRun?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchPipelineRun?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup?.groupVersion}/namespaces/${patchPipelineRun?.metadata?.namespace}/pipelineruns/${patchPipelineRun?.metadata?.name}`}
          kind="PipelineRun"
          name={patchPipelineRun?.metadata?.name || ''}

          cluster={cluster}
          content={patchPipelineRun}
        />
      </Drawer>
      <AICopilot
        view='list'
