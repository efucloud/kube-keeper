...
import { Button, Divider, Drawer, Modal, message, Popconfirm, Popover, Space, Tag, Typography, Empty, Select, theme } from 'antd';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<EventListener[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchEventListener, setPatchEventListener] = useState<EventListener>();
  const [dataSource, setDataSource] = useState<EventListener[]>([]);
  ...
  const listEventListeners = async () => {
    setLoading(true);
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'EventListener';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: EventListener[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: EventListener) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/eventlisteners/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<EventListener>[] = [
    {
...

    {
      ...
      render: (dom, entity: EventListener) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.tekton.EventListener.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('EventListener') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      header={{ breadcrumb: {} }}
      title={intl.formatMessage({ id: 'menu.DevOps.Tekton.eventListener' })}
      subTitle="EventListener"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<EventListener>
        key='event-listener'
        actionRef={actionRef}
        formRef={formRef}
        scroll={{ x: 'max-content' }}
        rowKey={(record: EventListener) =>
      ...
      <Drawer
        ...
        title={
          <>
            ...
            {patchEventListener?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchEventListener?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup?.groupVersion}/namespaces/${patchEventListener?.metadata?.namespace}/eventlisteners/${patchEventListener?.metadata?.name}`}
          kind="EventListener"
          name={patchEventListener?.metadata?.name || ''}

          cluster={cluster}
          content={patchEventListener}
        />
      </Drawer>
      <AICopilot
        view='list'
