...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Interceptor[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchInterceptor, setPatchInterceptor] = useState<Interceptor>();
  const formRef = useRef<ProFormInstance>(undefined);
  const [dataSource, setDataSource] = useState<Interceptor[]>([]);
  ...
  const resourceGroup = getClusterApiVersions(cluster, ['triggers.tekton.dev/v1alpha1'], 'Interceptor');
  const address = namespace ? `apis/${resourceGroup?.groupVersion}/namespaces/${namespace}/interceptors` : `apis/${resourceGroup?.groupVersion}/interceptors`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/devops/cicd/tekton/interceptors`);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Interceptor[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Interceptor) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup?.groupVersion}/namespaces/${entity.metadata?.namespace}/interceptors/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<Interceptor>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/devops/cicd/tekton/interceptors`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.tekton.Interceptor.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('Interceptor') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      header={{ breadcrumb: {} }}
      title={intl.formatMessage({ id: 'menu.DevOps.Tekton.interceptor' })}
      subTitle="Interceptor"
      content={<Continue current={currnetNumber} remainingItemCount={remainingItemCount} />}
    >
      <ProTable<Interceptor>
        key='interceptor'
        scroll={{ x: 'max-content' }}
        actionRef={actionRef}
        rowKey={(record: Interceptor) =>
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('Interceptor')}:&nbsp;&nbsp;
            {patchInterceptor?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchInterceptor?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup?.groupVersion}/namespaces/${patchInterceptor?.metadata?.namespace}/interceptors/${patchInterceptor?.metadata?.name}`}
          kind="Interceptor"
          name={patchInterceptor?.metadata?.name || ''}

          cluster={cluster}
          content={patchInterceptor}
        />
      </Drawer>
      <AICopilot
        view='list'
