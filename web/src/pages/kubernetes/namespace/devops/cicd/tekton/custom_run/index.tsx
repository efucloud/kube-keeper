import { CheckCircleFilled, CloseCircleFilled, CloseCircleOutlined, DeleteOutlined, EditOutlined, ExclamationCircleOutlined, InsertRowBelowOutlined, RedoOutlined, UnorderedListOutlined } from '@ant-design/icons';
import type { ActionType, ProColumns } from '@ant-design/pro-components';
import { FooterToolbar, PageContainer, type ProFormInstance, ProTable } from '@ant-design/pro-components';
import { Access, FormattedMessage, useAccess, useIntl } from '@umijs/max';
import { Button, Divider, Drawer, Modal, message, Popconfirm, Popover, Space, Spin, Tag, Typography, Select, Empty, theme } from 'antd';
...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<CustomRun[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchCustomRun, setPatchCustomRun] = useState<CustomRun>();
  ...
  const address = namespace ? `apis/${resourceGroup?.groupVersion}/namespaces/${namespace}/customruns` : `apis/${resourceGroup?.groupVersion}/customruns`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/devops/cicd/tekton/customruns`);
  ...
  const [dataSource, setDataSource] = useState<CustomRun[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: CustomRun[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: CustomRun) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup?.groupVersion}/namespaces/${entity.metadata?.namespace}/customruns/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<CustomRun>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/devops/cicd/tekton/customruns`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.tekton.CustomRun.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('CustomRun') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    <PageContainer
      header={{ breadcrumb: {} }}
      title={intl.formatMessage({ id: 'menu.DevOps.Tekton.customRun' })}
      subTitle="CustomRun"
    ...
    >
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('CustomRun')}:&nbsp;&nbsp;
            {patchCustomRun?.metadata?.name}
          </>
        }
      >
        <ResourceEditor
          key={patchCustomRun?.metadata?.resourceVersion || 'edit'}
          edit={editorResource}
          address={`apis/${resourceGroup?.groupVersion}/namespaces/${patchCustomRun?.metadata?.namespace}/customruns/${patchCustomRun?.metadata?.name}`}
          kind="CustomRun"
          name={patchCustomRun?.metadata?.name || ''}

          cluster={cluster}
          content={patchCustomRun}
        />
      </Drawer>
      <AICopilot
        view='list'
