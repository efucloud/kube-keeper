...
import { Button, Drawer, Dropdown, message, Popconfirm, Popover, Select, Space, Tag, Typography, Empty, Divider, theme } from 'antd';
...
const IndexDashboard: React.FC = () => {
  ...
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/access/rolebindings`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/access/rolebindings`);
  }
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/access/rolebindings`),
                );
              }}
            >
        ...
        if (!(record.metadata?.labels && record.metadata?.labels['kubernetes.io/bootstrapping'])) {
          nodes.push(
            <a
              key="edit"
              onClick={() => {
                window.location.href = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${record?.metadata?.namespace}/access/rolebindings/${record?.metadata?.name}/update`);
              }}
            >
