...
const IndexDashboard: React.FC = () => {
  ...
  let BaseAddress = `/kubernetes/cluster/${cluster}/namespace/${namespace}/access/rolebindings`;
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = `/kubernetes/cluster/${cluster}/access/rolebindings`;
  }
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  `/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/access/rolebindings`,
                );
              }}
            >
        ...
        if (!(record.metadata?.labels && record.metadata?.labels['kubernetes.io/bootstrapping'])) {
          nodes.push(
            <a
              key="edit"
              onClick={() => {
                window.location.href = `/kubernetes/cluster/${cluster}/namespace/${record?.metadata?.namespace}/access/rolebindings/${record?.metadata?.name}/update`;
              }}
            >
