...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<PrometheusRule[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchCertificate, setPatchCertificate] = useState<PrometheusRule>();
  ...
  const [dataSource, setDataSource] = useState<PrometheusRule[]>([]);
  ...
  const listPrometheusRules = async () => {
    setLoading(true);
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'PrometheusRule';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: PrometheusRule[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: PrometheusRule) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/prometheusrules/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<PrometheusRule>[] = [
    {
...

    {
      ...
      render: (dom, entity: PrometheusRule) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.PrometheusRule.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('PrometheusRule') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    ...
    >
      <ProTable<PrometheusRule>
        key='prometheus-rule'
        actionRef={actionRef}
        formRef={formRef}
        scroll={{ x: 'max-content' }}
        rowKey={(record: PrometheusRule) =>
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('PrometheusRule')}:&nbsp;&nbsp;
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchCertificate?.metadata?.namespace}/prometheusrules/${patchCertificate?.metadata?.name}`}
          kind="PrometheusRule"
