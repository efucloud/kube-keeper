...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Alertmanager[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchCertificate, setPatchCertificate] = useState<Alertmanager>();
  ...
  const address = namespace ? `apis/${resourceGroup?.groupVersion}/namespaces/${namespace}/alertmanagers` : `apis/${resourceGroup?.groupVersion}/alertmanagers`;
  let BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${namespace}/monitor/prometheus-operator/alertmanager`);
  if (!namespace || namespace === '' || namespace === '-') {
    BaseAddress = toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/monitor/prometheus-operator/alertmanager`);
  }
  const intl = useIntl();
  ...
  const [dataSource, setDataSource] = useState<Alertmanager[]>([]);
  ...
  const listAlertmanagers = async () => {
    ...
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'Alertmanager';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: Alertmanager[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Alertmanager) => {
      const params = { cluster, address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/alertmanagers/${entity.metadata?.name}` };
  ...
  const columns: ProColumns<Alertmanager>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/monitor/prometheus-operator/alertmanagers`),
                );
              }}
            >
...

    {
      ...
      render: (dom, entity: Alertmanager) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.Alertmanager.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('Alertmanager') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    ...
    >
      <ProTable<Alertmanager>
        key='alertmanager'
        actionRef={actionRef}
        formRef={formRef}
        scroll={{ x: 'max-content' }}
        rowKey={(record: Alertmanager) =>
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('Alertmanager')}:&nbsp;&nbsp;
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchCertificate?.metadata?.namespace}/alertmanagers/${patchCertificate?.metadata?.name}`}
          kind="Alertmanager"
