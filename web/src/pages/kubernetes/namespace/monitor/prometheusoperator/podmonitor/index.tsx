...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<PodMonitor[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchCertificate, setPatchCertificate] = useState<PodMonitor>();
  ...
  const address = namespace ? `apis/${resourceGroup.groupVersion}/namespaces/${namespace}/podmonitors` : `apis/${resourceGroup.groupVersion}/podmonitors`;
  ...
  const intl = useIntl();
  const resourceLabel = formatResourceKind(intl, 'PodMonitor');
  ...
  const [dataSource, setDataSource] = useState<PodMonitor[]>([]);
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: PodMonitor[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: PodMonitor) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/podmonitors/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<PodMonitor>[] = [
    {
        ...
        return (
          <>
            <a
              onClick={() => {
                window.open(
                  toKubernetesQueryRoute(`/kubernetes/cluster/${cluster}/namespace/${entity?.metadata?.namespace}/monitor/prometheus-operator/podmonitor`),
                );
              }}
            >
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.PodMonitor.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('PodMonitor') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchCertificate?.metadata?.namespace}/podmonitors/${patchCertificate?.metadata?.name}`}
          kind="PodMonitor"
