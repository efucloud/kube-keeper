...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<Prometheus[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchCertificate, setPatchCertificate] = useState<Prometheus>();
  ...
  const intl = useIntl();
  const resourceLabel = formatResourceKind(intl, 'Prometheus');
  ...
  const handleRemove = async (intl: IntlShape, selectedRows: Prometheus[]) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: Prometheus) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/prometheuses/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<Prometheus>[] = [
    {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.Prometheus.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('Prometheus') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
    ...
    >
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchCertificate?.metadata?.namespace}/prometheuses/${patchCertificate?.metadata?.name}`}
          kind="Prometheus"
