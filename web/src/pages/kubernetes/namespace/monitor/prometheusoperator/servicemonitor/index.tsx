...
const IndexDashboard: React.FC = () => {
  ...
  const [selectedRowsState, setSelectedRows] = useState<ServiceMonitor[]>([]);
  const [expandInfo, setExpandInfo] = useState<boolean>(false);
  const [patchCertificate, setPatchCertificate] = useState<ServiceMonitor>();
  ...
  const [dataSource, setDataSource] = useState<ServiceMonitor[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  useEffect(() => {
    listSericeMonitors();

  }, []);
  const listSericeMonitors = async () => {
    setLoading(true);
    try {
      ...
      for (let i = 0; i < data.items.length; i++) {
        ...
        data.items[i].kind = 'ServiceMonitor';
      }
  ...
  const handleRemove = async (
    intl: IntlShape,
    selectedRows: ServiceMonitor[],
  ) => {
    if (!selectedRows) return true;
    // 批量删除
    selectedRows.forEach(async (entity: ServiceMonitor) => {
      const params = {

        cluster,
        address: `apis/${resourceGroup.groupVersion}/namespaces/${entity.metadata?.namespace}/servicemonitors/${entity.metadata?.name}`,
      };
  ...
  const columns: ProColumns<ServiceMonitor>[] = [
    {
...

    {
      ...
      render: (dom, entity: ServiceMonitor) => {
        ...
        const nodes = [
          <Popconfirm
            ...
            description={intl.formatMessage({
              id: 'cluster.resource.ServiceMonitor.delete.description',
            })}
            title={
              intl.formatMessage({ id: 'pages.operation.delete.description' }) +
              getClusterResource('ServiceMonitor') + `【${record.metadata?.name}】`
            }
            onConfirm={() => {
  ...
  return (
    ...
    >
      <ProTable<ServiceMonitor>
        key='service-monitor'
        actionRef={actionRef}
        formRef={formRef}
        scroll={{ x: 'max-content' }}
        rowKey={(record: ServiceMonitor) =>
      ...
      <Drawer
        ...
        title={
          <>
            {getClusterResource('ServiceMonitor')}:&nbsp;&nbsp;
      ...
      >
        <ResourceEditor
          ...
          edit={editorResource}
          address={`apis/${resourceGroup.groupVersion}/namespaces/${patchCertificate?.metadata?.namespace}/servicemonitors/${patchCertificate?.metadata?.name}`}
          kind="ServiceMonitor"
