...
func generateTypescriptDefine(projectRoot, outputDir string) error {
	dtoFiles, err := collectDTOFiles(path.Join(projectRoot, "pkg", "models", "dtos"))
	if err != nil {
		return err
	}
	...
	for _, dtoFile := range dtoFiles {
		item, parseErr := goparser.ParseFile(fileSet, dtoFile, nil, goparser.DeclarationErrors|goparser.ParseComments)
		if parseErr != nil {
			return parseErr
		}
		filename := strings.TrimSuffix(filepath.Base(dtoFile), ".go")
		...
		for _, need := range needImports {
			if !common.StringInArray(need, fileNeeds[filename]) {
				fileNeeds[filename] = append(fileNeeds[filename], need)
			}
		}
		...
		if needs := fileNeeds[n]; len(needs) > 0 {
			for _, need := range needs {
				if entryFile, ok := allEntries[need]; ok {
					if !common.StringInArray(need, importFiles[entryFile]) {
						importFiles[entryFile] = append(importFiles[entryFile], need)
					}
				}
			}
		}
		importFileNames := make([]string, 0, len(importFiles))
		for fileName := range importFiles {
			importFileNames = append(importFileNames, fileName)
		}
		sort.Strings(importFileNames)
		for _, k := range importFileNames {
			if k == fmt.Sprintf("./%s.d", n) {
				continue
			}
			v := importFiles[k]
			sort.Strings(v)
			needStr += fmt.Sprintf("import { %s } from '%s';\n", strings.Join(v, ", "), k)
		}
		if n == "common" {
			needStr = ""
		}
		if err = os.WriteFile(path.Join(outputDir, fmt.Sprintf("%s.d.ts", n)), []byte(needStr+f), os.ModePerm); err != nil {
			return err
		}
	}
	// 外部结构体导入
	...
	converter.Indent = "    "
	converter.BackupDir = ""
	converter.Add(gitlab.ListOptions{})
	converter.Add(metav1.APIResource{})
	converter.Add(metav1.Verbs{})
	err = converter.ConvertToFile(path.Join(outputDir, "external.d.ts"))
	if err != nil {
	...
	if err = os.WriteFile(path.Join(outputDir, "entries.json"), data, os.ModePerm); err != nil {
		return err
	}
	//// 生成导出文件
	//exportContent := ""
	//for k, v := range fileStructs {
	//	exportContent += fmt.Sprintf("import { %s } from '%s';\n", strings.Join(v, ","), k)
	//}
	//exportContent += "\n//导出\nexport {\n"
	//for _, v := range fileStructs {
	//	exportContent += fmt.Sprintf("  %s\n", strings.Join(v, ","))
	//}
	//exportContent += "};"
	//_ = os.WriteFile(path.Join(dir, "generates", "data.d.ts"), []byte(exportContent), os.ModePerm)
	return nil
}

func collectDTOFiles(dtoDir string) ([]string, error) {
	files, err := filepath.Glob(path.Join(dtoDir, "*.go"))
	if err != nil {
		return nil, err
	}
	sort.Strings(files)
...
func extractFieldInformationFromStructField(structField *ast.StructType, fileUnresolveds map[token.Pos]*ast.Ident) (fi FieldInformation) {
	fields := extractFieldsFromStructType(structField, fileUnresolveds, nil)
	if len(fields) > 0 {
		return fields[0]
	}
	return
