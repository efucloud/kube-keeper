...
func (api ApiData) GenerateAPI() (content string, entries, functions []string) {
	...
	if len(api.ResponseModel) > 0 && api.ResponseModel != "string" {
		if common.StringInArray(api.ResponseModel, []string{"ArrayString", "ArrayUint"}) {

		} else {
			content += fmt.Sprintf("<%s>", api.ResponseModel)
	...
	if api.ResponseModel == "ArrayString" {
		api.ResponseModel = "string[]"
	} else if api.ResponseModel == "ArrayUint" {
...
func (api ApiData) Params() string {
	...
	if api.HasQueryParams || api.HasPathParams || len(api.FormParams) > 0 {
		...
		for _, name := range api.parameterNames() {
			...
			} else if item.Position == "multipart/form-data" {
				fmt.Println("wer")
			}
		}
		content += pathParam
...
func (rest *RestAPI) GenerateToDir(dir string) {
	...
	rest.loadEntrySource(dir)
	fileNames := make([]string, 0, len(rest.files))
	for fileName := range rest.files {
...
func (rest *RestAPI) loadEntrySource(dir string) {
	entryFiles := []string{path.Join(dir, "entries.json")}
	if cwd, err := os.Getwd(); err == nil {
		entryFiles = append(entryFiles, path.Join(cwd, "generates", "entries.json"))
	}
